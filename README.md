...
- **Open PRs keep moving on their own** — the opt-in `pr-watch` row polls your open PRs, and when a review bot or a person comments or CI fails, a subagent in its own worktree replies, fixes, pushes, and resolves bot threads (never people's). Every session shows the PRs being worked under its composer; `/prs` lists them.
