...
- **Open PRs keep moving on their own** — the opt-in `pr-watch` row finds your open PRs on GitHub from any session, and when a review bot or a person comments or CI fails, a subagent in a worktree of the watcher's own clone replies, fixes, pushes, and resolves bot threads (never people's). Nothing reaches the transcript: the status bar shows a bright count of PRs being worked by any session, and `/background` prints the jobs' steps.
- **The attention board** — `/current-work` opens a live web page (with `attention: {config: {web: localhost:7683}}`) or pins a view at the top of the screen, showing the work around you from the memory graph. The page is a flow board: one row per subject (the ticket a PR implements), grouped by whose turn it is (needs me, in motion, blocked, waiting on others), each row's last 1/7/30 days as stage-coloured segments swept from the graph's timeline (queued, building, in review, blocked, shipping, with an agent overlay while a session or pr-watch job holds it), the graph's edges as marks, the card as it is now, and what will touch it next (the pr-watch poll, the collector, a nudge). Rows link to GitHub or Linear, hovering a card shows the graph's neighbourhood, and it re-reads the graph every five seconds. Chats are URLs on the same page: `/s/<session id>` attaches the browser to that session (the main web session, or a second `bough --web` the page starts beside it on a free port and reuses), `/s/new?draft=…` starts a chat with the composer prefilled, `/sessions` lists recent chats with live markers, and every session mark and hover line links to its chat. `/work-report` is the model's written version.
- **cmux tabs take the session's name** — inside a [cmux](https://cmux.com) terminal the sidebar tab is renamed to the session title as soon as bough picks one, and to the title on file when a session is resumed. Nothing happens outside cmux.
...
| `/model` | every model of every provider, searchable — type to filter, `enter` switches; `/model refresh` refetches the catalogue |
...
| `esc` `esc` | clear the draft (`↑` brings it back); on an empty composer, open the rewind tree — every turn of this session, the sessions it was forked from and every fork off them; `enter` on a turn goes back to before it, `enter` on a branch's `(continue)` switches to that branch |
