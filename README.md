...
- **The attention board** — `/current-work` pins a live view of the work around you at the top of the screen, from the memory graph: NEEDS ME, IN MOTION (a session is on it), WAITING ON OTHERS. The bar is how long it has been that party's turn, dependency bumps fold into one row, changed rows brighten, and it re-reads the graph every five seconds. `attention: {config: {sticky: true}}` in your `~/.bough/bough.yml` shows it from the first frame of every session; `/work-report` is the model's written version.
