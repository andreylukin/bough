...
func (c *controlLLM) text(ctx context.Context, child bool, users []string, onDelta func(string)) (string, error) {
	...
	name, turn, ok, err := c.take(ullm.Request{}, child, users)
	if err != nil {
		return "", err
	}
	if !ok {
...
// reached the model.
func (c *controlLLM) take(r ullm.Request, child bool, users []string) (name string, t controlTurn, ok bool, err error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (a *controlAdapter) Respond(ctx context.Context, r ullm.Request, o ullm.RequestOptions) (ullm.Response, error) {
	...
	name, turn, ok, err := a.c.take(r, a.opts.Worker != "", requestUsers(r))
	if err != nil {
		return ullm.Response{}, err
	}
