...
type controlTurn struct {
	...
	// Tool and Args make a "call" release answer with that tool call:
	// the engine's ask is how a test takes a session to "needs-you".
	...
	case "block":
		...
		for {
			if b, err := os.ReadFile(release); err == nil {
				...
				if then.Mode == "call" {
					return a.call(ctx, name, then.Tool, then.Args), nil
				}
				if then.Text != "" {
...
// call answers with one tool call, as a provider does when the model
// uses a tool; the engine runs it and asks again with its result.
func (a *controlAdapter) call(ctx context.Context, name, tool, args string) ullm.Response {
	if args == "" {
		args = "{}"
	}
	id := "control_" + name
	if a.opts.Sink != nil {
		a.opts.Sink(agentllm.Delta{Seq: agentllm.SeqOf(ctx), Attempt: 1, Kind: agentllm.DeltaToolStart, CallID: id, Name: tool})
	}
	u := ullm.Usage{InputTokens: 1, OutputTokens: 1}
	a.c.mu.Lock()
	addAgentUsage(&a.c.usage, u)
	...
	return ullm.Response{
		ID:     fmt.Sprintf("control-%d", n),
		Stop:   ullm.StopComplete,
		Output: []ullm.Item{{Type: ullm.ItemToolCall, Data: ullm.ToolCall{CallID: id, Name: tool, Arguments: args}}},
		Usage:  u,
	}
}
