...
// Podman only runs on Linux, and the bind-mount owner check reads
// syscall.Stat_t, which windows lacks.
...
// TestPodmanLive exercises real rootless podman; it needs network for
// the base image, so it only runs on request. Same script as TestAppleLive.
func TestPodmanLive(t *testing.T) {
	t.Parallel()
	if os.Getenv("BOUGH_LIVE_CONTAINER") != "1" {
		t.Skip("set BOUGH_LIVE_CONTAINER=1 to run against podman")
	}
	...
	t.Cleanup(func() {
		...
		_ = exec.Command(a.Bin, "image", "rm", tag).Run()
		_ = exec.Command(a.Bin, "volume", "rm", vol).Run()
	})
