...
func (a *API) sessionOrbLog(w http.ResponseWriter, r *http.Request) {
	...
	if st.Phase == orb.PhaseBuild && st.Project != "" {
	...
	writeJSON(w, http.StatusOK, map[string]any{"status": st.Status, "error": st.Error, "phase": st.Phase, "image": st.Image, "log": name, "text": text})
}
