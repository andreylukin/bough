...
export interface OrbSummary { slug: string; image: string; built: boolean; build?: string; error?: string; /** Repo names project.yml declares. */ repos?: string[]; /** Changes with any definition file, so the orb panel re-reads on an edit made elsewhere. */ files?: string }
