...
test("a build failure shows the error lines, links to Projects → Orb and offers only Rebuild", () => {
  const html = renderToStaticMarkup(
    <OrbFailureBody projectSlug="web" name="Web" onRebuild={noop} onRetry={noop}
  ...
  expect(html).toContain('href="#/projects/web/orb"');
...
test("the picker warns about a project whose last image build failed", () => {
  const projects = [{ slug: "web", name: "Web", orb: { slug: "web", image: "", built: false, build: "failed" } }] as Project[];
  expect(failedBuild(projects[0])).toBe(true);
  const html = renderToStaticMarkup(<ModePicker projects={projects} value={{ mode: "project", project: "web" }} onChange={noop} />);
