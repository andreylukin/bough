...
func (s *Supervisor) CreateChild(opt CreateOptions, maxPerSession, maxRunning int) (id string, queued bool, err error) {
	...
	m.Project = slug
	if q.prompt != "" || len(s.running) >= s.maxRunning {
		// Also for a child started now: until its turn is under way, a
		// restart must start it again (drainQueue keeps it the same way).
...
// An exit before any turn is a child that never started one; stopped
// says a Stop killed it.
