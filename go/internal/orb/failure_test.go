...
func TestEnsureImageFailureCarriesLogLines(t *testing.T) {
	...
	if FailedAt(st) != PhaseBuild || !strings.Contains(st.Error, "Unable to locate package") {
		t.Fatalf("state %+v", st)
	}
}
...
func TestOpenResumeFailureCarriesLogLines(t *testing.T) {
	...
	st := o.State()
	if FailedAt(st) != PhaseSetup || !strings.Contains(st.Error, "npm ERR! missing script: dev") || strings.Contains(st.Error, "== resume.sh") {
		t.Fatalf("state %+v", st)
	}
}
...
func TestOpenStartFailurePhase(t *testing.T) {
	...
	if st, _ := ReadState(home, "s7"); FailedAt(st) != PhaseStart {
		t.Fatalf("state %+v", st)
	}
}
