...
func newASAdapter(t *testing.T) *asAdapter {
	...
	return &asAdapter{
		t: t, s: s, dir: dir, flags: s.Dir(t, "flags"), stats: map[string]int{},
...
func agentStopPathsHistory(entries []history.Entry) []tracecheck.Step {
	...
	for _, e := range entries {
		...
		case "input":
			...
			open, lastClose = true, ""
			switch {
			case e.Data["wake"] == true && wakeNext:
				wakeNext = false
			case turns == 1:
...
func TestAgentStopPathsPaths(t *testing.T) {
	...
	for sh := range shards {
		t.Run(fmt.Sprintf("shard%d", sh), func(t *testing.T) {
			...
			for _, id := range a.as {
				checkHistory(t, g, sessionHistory(t, a.s.Home, id), agentStopPathsHistory)
			}
		})
	}
}
