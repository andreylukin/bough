...
// walkPath drives one generated path and compares the whole state after
// every step; the first difference ends the path.
func walkPath(a *askAnswerAdapter, path []tracecheck.Step) error {
	defer a.Cleanup()
	...
	for s := range shards {
		t.Run(fmt.Sprint(s), func(t *testing.T) {
			...
			for i := s; i < len(paths); i += shards {
				if err := walkPath(a, paths[i]); err != nil {
					var acts []string
