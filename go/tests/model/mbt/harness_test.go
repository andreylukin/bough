...
// the process dies. The lock file is at a fixed path, not under
// os.TempDir(): the port is machine-wide, and two runs with their own
// TMPDIR each took their own lock and failed on the port.
