...
// the process dies. The lock lives in /tmp, not os.TempDir(): a run
// given its own TMPDIR (a per-job sandbox, a CI step) would otherwise
// lock its own file and collide on the port with every other run.
