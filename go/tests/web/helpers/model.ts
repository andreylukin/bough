  ...
  test.describe(`model: ${flow.spec}`, () => {
    ...
    loadPaths(flow.spec).forEach((trace, i) => {
      ...
      const run = async (serve: Serve, page: Page, info: TestInfo) => {
        const errors: string[] = [];
        // Chromium logs every non-2xx fetch as an error. A 4xx is a
        // refusal the flow walks on purpose (a 409, a 400) and the page
        // shows it; a 5xx or anything the page itself logs still counts.
        page.on('console', (m) => {
