...
function saveTranscripts(serve: Serve, spec: string, ids: string[], title: string): void {
  ...
  // Capped: an exhaustive walk's title runs to 50 steps, past the 255-byte
  // file name limit (ENAMETOOLONG, thrown from the finally over the walk's
  // own error). The session id keeps the names unique.
