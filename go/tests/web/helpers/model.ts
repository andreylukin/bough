...
function saveTranscripts(serve: Serve, spec: string, ids: string[], title: string): void {
  ...
  // Capped: a MODEL_COVER=transitions path names fifty actions, past the
  // 255-byte file name limit (ENAMETOOLONG, which, thrown from the walk's
  // finally, also hid whether the walk itself passed). "path-<n>" leads,
  // so the name stays unique.
