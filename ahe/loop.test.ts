...
const change_ = (predicted_pass: string[]): ChangeEntry => ({
...
test("with a saturated bank, a named waste metric settles the edit", () => {
  // Every task passes, so there is no flip to settle against. AHE would have no
  // verdict here; this loop falls back to the metric the edit itself named.
  const change = { ...change_(["nothing"]), predicted_waste: "parseErrors" as const };
  const before = { ...result({ alpha: [3, 3] }), parseErrors: 6 };
...
test("a tidier trace does not excuse a lost task", () => {
  // The failure mode this forecloses: an edit that regresses a task while making
  // the traces look cleaner, and a loop that reads the second fact and keeps it.
  const change = { ...change_(["alpha"]), predicted_waste: "parseErrors" as const };
  const v = settle(
    [change],
    { ...result({ alpha: [3, 3] }), parseErrors: 6 },
    { ...result({ alpha: [0, 3] }), parseErrors: 0 },
  );
  strictEqual(v[0].held, false);
