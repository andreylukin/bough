...
if ! diff -q "$TASK/fixture/lru/api.py" "$WS/lru/api.py" >/dev/null 2>&1; then
  fail "lru/api.py was modified (it is protected)"
fi

# 2. The checked-in suite still passes — no fixing the hidden cases by breaking
#    the visible ones.
if ! (cd "$WS" && python3 -m unittest test_cache -v) >"$WS/.visible.log" 2>&1; then
  fail "the checked-in test_cache.py no longer passes"$'\n'"$(tail -20 "$WS/.visible.log")"
fi
