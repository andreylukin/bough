...
if ! diff -q "$TASK/fixture/tok/api.py" "$WS/tok/api.py" >/dev/null 2>&1; then
  fail "tok/api.py was modified (it is protected)"
fi

# 2. The checked-in suite still passes — no fixing the hidden cases by breaking
#    the visible ones.
if ! (cd "$WS" && python3 -m unittest test_tok -v) >"$WS/.visible.log" 2>&1; then
  fail "the checked-in test_tok.py no longer passes"$'\n'"$(tail -20 "$WS/.visible.log")"
fi
