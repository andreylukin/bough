...
#
# Outcome-graded: this reads the code, never the transcript. Three gates, and the
# order is deliberate — a protected-file violation is reported as itself rather
# than as whatever test failure it happens to produce.
set -u
WS="$1"
...
if ! diff -q "$TASK/fixture/pipe/legacy.py" "$WS/pipe/legacy.py" >/dev/null 2>&1; then
  fail "pipe/legacy.py was modified (it is protected)"
fi

# 2. The checked-in suite still passes — no fixing the hidden cases by breaking
#    the visible ones.
if ! (cd "$WS" && python3 -m unittest test_pipe -v) >"$WS/.visible.log" 2>&1; then
  fail "the checked-in test_pipe.py no longer passes"$'\n'"$(tail -20 "$WS/.visible.log")"
fi
