...
if ! diff -q "$TASK/fixture/wire/api.py" "$WS/wire/api.py" >/dev/null 2>&1; then
  fail "wire/api.py was modified (it is protected)"
fi

# 2. The checked-in suite still passes — no fixing the hidden cases by breaking
#    the visible ones.
if ! (cd "$WS" && python3 -m unittest test_wire -v) >"$WS/.visible.log" 2>&1; then
  fail "the checked-in test_wire.py no longer passes"$'\n'"$(tail -20 "$WS/.visible.log")"
fi
