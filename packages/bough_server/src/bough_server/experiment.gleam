...
fn run_escalate_one(
  ...
  let t0 = clock.now_ms()
  // Tier 1: fast executor, best-of-N one-shots. The 3B has high per-shot
  // variance; a second sub-second sample is far cheaper than escalating to the
  // relay or frontier, so harvest the variance here (BOUGH_EXP_TIER1_SAMPLES).
  let #(t1_solved, t1_n) =
    tier1_bestof(exec_url, exec_model, ws, task, env_int("BOUGH_EXP_TIER1_SAMPLES", 1), 1)
  let #(solved, outcome, rounds, execs) = case t1_solved {
    True -> #(True, "tier1", 1, t1_n)
...
/// Run the executor up to `n` independent one-shots, resetting the fixtures
/// before each retry so every sample is fresh, and stopping at the first that
/// passes the ground-truth check. Cheap variance-harvesting: a second ~sub-second
/// sample beats escalating to the relay/frontier. Returns #(solved, attempts).
fn tier1_bestof(
  url: String,
  model: String,
  ws: String,
  ...
  let _ = executor_oneshot(url, model, ws, task)
  case check_passes(ws, task.ground_truth) {
    True -> #(True, attempt)
    False ->
      case attempt >= n {
        ...
        False -> {
          write_fixtures(ws, task.fixtures)
          tier1_bestof(url, model, ws, task, n, attempt + 1)
        }
      }
  }
}
  ...
  case iter > max_iters {
    ...
    False -> {
      let r_user =
        ...
        <> current_files(ws, task.fixtures)
        <> history
      // Reasoner: VibeThinker decoding (temp 1.0 / top_p 0.95). The CoT budget is
      // the latency-tail lever — BOUGH_EXP_REASONER_MAXTOK caps it so a hard task
      // defers to the frontier fast instead of grinding a long local CoT.
      case
        worker.complete_with(
          ...
          reasoner_system,
          r_user,
          env_int("BOUGH_EXP_REASONER_MAXTOK", 6000),
          option.Some(1.0),
          option.Some(0.95),
        )
      {
