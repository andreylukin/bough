...
- {\"action\":\"code\", \"title\":..., \"code\":\"<Python>\"} — your primary action. Write a Python program that calls these host functions and prints what matters:
    ...
    - mcp(server_cmd, tool, args_json) -> str : spawn an MCP stdio server via `sh -c server_cmd`, call `tool` with the parsed `args_json` object (JSON-RPC 2.0), and return the tool result text
