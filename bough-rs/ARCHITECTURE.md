# bough-rs — Architecture (binding)

This document is the binding design for the Rust/ratatui rewrite. Porters follow it
without asking. Per-module contracts live in `specs/*.md` (16 files) — this document
decides the workspace shape, the crate boundaries, the shared-types strategy, the
Bun-replacement decisions, and the concurrency model. Where this document and a spec
disagree on structure, this document wins; where they disagree on *behavior*, the spec
wins (behavior is pinned by the TS test suite).

## 0. The parity anchor

**The server/TUI split over loopback HTTP + SSE is preserved exactly as the TS system
has it.** Same routes, same JSON field names, same status codes (202 for postMessage,
201 for creates), same SSE framing (`event:` + single `data:` line, no `id:` field,
`: connected` / `: ping` comments). This is the anchor that makes the port verifiable
incrementally:

- the **Rust TUI must run against the TS server** (`bun run dev` + `bough-rs tui`), and
- the **TS TUI must run against the Rust server** (`bough-rs start` + `bun run tui`),

at every wave boundary. `BOUGH_PORT` (default 4321, loopback only, no CORS ever) and
`BOUGH_HOME` (default `~/.bough`) let a dev instance run beside the live install.
The full route table is `specs/server.md` §3 — that file IS the API contract.
