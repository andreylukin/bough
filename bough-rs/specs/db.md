...
**Stub / drop in v1:**
- The three migrate reshapes: **keep `addScheduleSessionId` + `addCommandMessageId`** (trivial, and the user's live bough.db on two installs depends on them at first open by bough-rs); `rebuildDayOneCommandHistory` can be kept too (10 lines) — dropping it risks a wedge on any file older than 2026-08, so just port all three.
