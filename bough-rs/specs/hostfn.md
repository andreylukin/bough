...
- `interface SpillDeps { exists, mkdirp, write, append }` — injected filesystem.
...
- `spillMarker(path, total, lines, omitted) -> String` — names the path, true total, and three runnable follow-ups (`rg -n 'error|fail' '<path>'`, `bough patterns --llm '<path>'`, `view("<path>")`), ends "Do not re-run the command to see the middle". Paths shell-quoted.
- `spill(text, ctx{scratch?, label?, sink?}, deps) -> String` — bound text for a tool result. With a sink: use THAT file and THAT total (the in-memory text is the retained size, not the real one). Without: plan → write `<label>-NNN.log` → head+marker+tail, or fall back to `truncateMiddle` on no scratch / write failure. `nextPath` probes `label-001.log`…`999` for the first free name (a counter resets across restarts and would overwrite); slot 999 is reused explicitly.
