...
use bough_kernel::{Kernel, SerialEvent, WaterfallEvent};
use bough_plugin_hello::trace;
use support::{boot_real, fixture, maybe_row, TempDir};

/// The events this test watches. Every one is dispatched by a row the disable/re-enable walk takes
/// down and puts back, so a listener the walk failed to dispose shows up as a count that grew.
fn watched() -> Vec<&'static str> {
    vec![
        ...
        bough_plugin_llm::LlmStreamEvent::NAME,
    ]
}
