...
entries:
  # H-C5 is CLOSED at the merge, so the row patch that used to be here is gone. `spawn_worker`
  # runs on the LOOP's context (`ToolCx.ctx`) and `agent-loop` now declares `workers` as an
  # optional key of its own, so the SHIPPED tree can spawn a worker through the tool surface —
  # `crates/bough/tests/workers_seam.rs` is the gate. This transcript therefore runs against the
  # shipped inject, which is the only way it can measure the shipped bounds.
  llm.anthropic:
    plugin: llm-replay
    config:
      strict: false
