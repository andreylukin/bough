...
///
/// `agent:` is OPTIONAL, and it WORKS at every site now. MERGE (H-C6 closed): the wake-stopping
/// listener used to filter with `AgentName::new(p.agent.as_str())`, and `AgentWakeStopping`'s
/// `agent` is an `AgentId` (a uuid), so a name filter matched nothing there and every configured
/// fault fired on every agent. The name comes off the live handle the payload carries;
/// `an_agent_filter_matches_by_name_at_the_wake_stopping_site` is the gate.
fn fault_row(at: &str, how: &str, after: u32, times: u32, needs: &[&str]) -> String {
    fault_row_for(at, how, after, times, needs, None)
}

/// [`fault_row`] with an `agent:` filter.
fn fault_row_for(
    at: &str,
    how: &str,
    after: u32,
...
/// H-C6, both sides: an `agent:` filter that NAMES the agent fires at `wake_stopping`, and one
/// that names a different agent does not. Before the merge neither happened — the filter compared
/// a name to a uuid, so it never matched and the site behaved as though no filter were set.
#[tokio::test(flavor = "multi_thread")]
async fn an_agent_filter_matches_by_name_at_the_wake_stopping_site() {
    async fn fired_for(agent: &str) -> bool {
        let _guard = trace::test_lock();
        let _faults = bough_plugin_fault_inject::test_lock();
        let layer = Layer::new(
            &format!("filter-{agent}"),
            &fault_row_for("wake_stopping", "panic", 1, 1, &["agents"], Some(agent)),
        );
        let (kernel, _dir) =
            boot_real("headless", &[fixture("fault.patch.yml"), layer.path()]).await;
        ...
        kernel
            ...
            .await
            .expect("a listener registers");
        run_wakes(&kernel, 1).await;
        kernel.shutdown().await;
        let seen = seen.lock().expect("not poisoned").clone();
        seen.iter().any(|e| e.contains("agent/wake-stopping"))
    }
