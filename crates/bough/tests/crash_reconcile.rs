...
fn open(home: &Home) -> Arc<SqliteStore> {
    ...
    for def in bough_plugin_agents::vocabulary::step_types()
        ...
        .chain(bough_plugin_mail_router::vocabulary::step_types())
        .chain(bough_plugin_graph_ops::vocabulary::step_types())
        .chain(bough_plugin_dormancy::vocabulary::step_types())
        .chain(bough_plugin_drift_watch::vocabulary::step_types())
        .chain(bough_plugin_reconsolidation::vocabulary::step_types())
        .chain(bough_plugin_worker_fork::vocabulary::step_types())
        .chain(bough_plugin_leader::vocabulary::step_types())
        .chain(bough_plugin_about_line::step_types())
        .chain(bough_plugin_rollups_summarizer::step_types())
        .chain(bough_plugin_llm::usage::step_types())
    {
...
fn interrupted_wake(steps: &[Step]) -> (WakeId, &Step) {
    let end = steps
        ...
        .expect("boot repair closed the orphaned wake with the one reason no live loop emits");
    (end.wake.clone(), end)
}
