...
async fn resident(kernel: &Kernel, name: &str) -> (bough_plugin_agents::Agent, TrajId) {
    ...
    // MERGE: WAITED FOR, and RE-PEEKED every pass. Two facts made a single read a coin flip.
    // `residents` raises the roster asynchronously AFTER the row activates (the same window merge
    // note 16 measured from the composer's end), and the registry entry and the `agents` LEDGER
    // row do not land in the same instant — so a booted, quiesced tree guarantees neither. And
    // `peek_live` answers with the registry that is bound RIGHT NOW: a boot-time reload of the
    // `agents` row replaces it, and a handle captured before that reload stays empty for ever,
    // which is what "live: []" after five seconds of polling turned out to be. The panic below
    // still fires for a tree that genuinely has no `sol`.
    let mut found = None;
    for _ in 0..200 {
        let agents = kernel.root().peek_live::<Agents>();
        let ledger = kernel.root().peek_live::<Ledger>();
        if let (Some(agents), Some(ledger)) = (agents, ledger) {
            if let Some(agent) = agents.by_name(&agent_name) {
                if let Some(row) = ledger.0.agent(&agent_name).await.expect("a read") {
                    found = Some((agent, row.traj));
                    break;
                }
            }
        }
    ...
    found.unwrap_or_else(|| {
        let live: Vec<String> = kernel
            .root()
            .peek_live::<Agents>()
            .map(|a| a.list().iter().map(|a| a.name().to_string()).collect())
            .unwrap_or_default();
        panic!("the shipped tree has no resident `{name}` with a ledger row; live: {live:?}")
    })
}

/// Link one lane to `repo:o/r`, which is what the collector's items carry as their SCOPE ref.
/// The router routes on that; a lane that asked for nothing gets nothing.
async fn link_repo(kernel: &Kernel, name: &str) {
    kernel
        .root()
        .peek_live::<bough_plugin_mail_router::Mail>()
        .expect("`mail` is bound")
        .link_ref(
            &AgentName::new(name),
            std::collections::BTreeSet::from([bough_plugin_ledger::Ref::new("repo:o/r")]),
            chrono::Utc::now(),
        )
        .await
