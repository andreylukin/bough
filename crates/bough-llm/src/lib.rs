//! The provider layer (port of bough's `src/llm/`). No provider name appears
//! outside this crate. The Anthropic client is hand-rolled reqwest + SSE (no
//! SDK); the SSE parser stays hand-rolled — the `[DONE]`/stall/
//! trailing-fragment semantics are custom and test-pinned. Provider-specific
//! handling must not leak past `types::LlmClient`.
//!
//! The invariant: **the caller must not know which provider it is talking
//! to.** Three wire protocols, three message encodings, three usage shapes,
//! three ways of admitting that a stream died — all of it collapses to one
//! `run()`.
//!
//! Entry points: [`client_for`] (routed + priced + traced + retried),
//! [`provider_client`] (bare), [`complete_text`] (one-shot text).

// Same lint posture as bough-core: the injection seams are `Arc<dyn Fn(..)>`
// fields, and the module headers carry hand-aligned definition lists.
...
pub mod test_support {
    ...
    use crate::sse::{body_of, HttpRequest, HttpResponse, Transport};
    use crate::types::{
    ...
    pub fn keyed_env() -> crate::routing::Env {
    ...
    impl LlmClient for FakeClient {
        async fn run(
            ...
            if cancel.is_cancelled() {
                return Err(crate::sse::aborted("fake"));
            }
            let next = self.script.lock().unwrap().pop_front();
            match next {
                None => Err(LlmError::new("fake: script exhausted")),
    ...
    #[allow(clippy::type_complexity)]
    pub fn fake_client(
        script: Vec<Result<LlmResult, LlmError>>,
    ...
    impl Transport for CannedTransport {
        async fn fetch(&self, req: HttpRequest) -> Result<HttpResponse, LlmError> {
            ...
            let Some(next) = next else {
                return Err(LlmError::new("CannedTransport: no response queued"));
            };
