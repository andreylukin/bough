...
const HORIZON = 6 * 3_600_000; // what gets used in the next six hours of work
...
for (const [repo, rs] of byRepo) {
  if (rs.length < 200) continue; // a repo with no history has nothing to rank
  const t0 = rs[0].ts, tEnd = rs[rs.length - 1].ts;
  // Cuts every two hours across the repo's life, each needing past and future.
  for (let cut = t0 + 6 * 3_600_000; cut < tEnd - HORIZON; cut += 2 * 3_600_000) {
    const past = rs.filter((r) => r.ts <= cut && r.ts > cut - 150 * DAY);
    const future = new Set(
      rs.filter((r) => r.ts > cut && r.ts <= cut + HORIZON && !isRef(r.tag)).map((r) => r.tag),
    );
    if (past.length < 50 || future.size === 0) continue;
    cuts++;
...
console.log(`cuts: ${cuts}  repos: ${byRepo.size}  horizon: ${HORIZON / 3_600_000}h  K=${K}`);
console.log("model  precision@10  recall@10   MRR");
for (const m of models) {
  const p = (hits[m] / cuts * 100).toFixed(1);
  const r = (recall[m] / cuts * 100).toFixed(1);
  const mr = (mrr[m] / cuts).toFixed(3);
  console.log(`${m.padEnd(6)} ${p.padStart(11)}% ${r.padStart(9)}% ${mr.padStart(7)}`);
}
