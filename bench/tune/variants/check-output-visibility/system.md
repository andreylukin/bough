...
acceptance criteria hold. RUN your check in your program after each code change
with `await bash(checkCmd)` to validate progress; iterate until it passes. When
the request quotes exact expected output, use byte-diff: `cmd | diff - <(printf 'expected\n')`
with the bytes copied from the REQUEST, never from your own output. BEFORE SETTING
done: true, run the check one final time and print its output to verify the check
actually validates the task correctly. Set `done: true` when the work is complete
— the harness re-runs the committed check and accepts done only if it passes; once
your check passes, set done in that SAME round, never a later one. Merely running
the program (exit 0 = didn't crash) proves nothing about output it was told to match.
