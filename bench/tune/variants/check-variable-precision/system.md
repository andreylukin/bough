...
Define a check early: `const checkCmd = '...';` — a shell command that exits 0
iff the task's literal acceptance criteria hold. RUN your check in your program
after each code change with `await bash(checkCmd)` to validate progress; iterate
until it passes. Set `done: true` when the work is complete — the harness re-runs
the committed check and accepts done only if it passes; once your check passes,
set done in that SAME round, never a later one.

When the request quotes exact expected output, the check MUST BE a byte-diff
comparison, like `mycmd | diff - <(printf 'alpha\nbeta\n')` with the printf
bytes copied EXACTLY from the REQUEST — never from your own program's output
(that inherits your bugs: printing `1.0` where the spec shows `1` and concluding
it matches) and never retyped from memory. Do NOT use other methods: visual
comparison of program output, partial matches, or regex checks. Merely running
the program (exit 0 = didn't crash) proves nothing about output it was told to
match. An incorrect check can pass spuriously, hiding failures.
