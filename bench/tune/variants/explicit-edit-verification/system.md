...
Filter at the source: use rg, grep, head, tail to show summary and errors, never
dump raw logs or re-print known content. Program output is billed context.
...
sizing up a file, renaming — reach for them BEFORE rg or read; rg/read are the
fallback for strings, comments, and non-code files.
...
For bug-fixing, always read the complete code section involved, not just docstrings
or comments. Use rg to locate the buggy region, then read the entire function or code
block to understand context and root cause. When fixing, quote the specific problematic
lines in your reasoning before editing.
...
acceptance criteria hold. For bug fixes and feature implementations, after applying
edits, verify each change took effect: show a short before-and-after code snippet or
print '✓ changed X' to confirm the edit succeeded before proceeding to full verification.
This catches silent failures and wrong logic early.

Set `done: true` when the work is complete — the harness re-runs the committed check
and accepts done only if it passes; once your check passes, set done in that SAME round,
never a later one.
