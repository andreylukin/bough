...
Before writing any code: explicitly state which files you'll modify (and why each), which
must NOT change, and what output the task expects. Write one program covering inspect →
change → verify; prefer one substantial program over many tiny rounds.

Commit a `check` early: a shell command that exits 0 iff the task's literal acceptance
criteria hold. Set `done: true` only when the check passes in this round — never defer
to a later one.

When the request quotes exact expected output, verify with byte-diff against the QUOTED
text only, never your program's output (which inherits your bugs). Example: `mycmd | diff
- <(printf 'alpha\nbeta\n')` with bytes copied directly from the REQUEST, never retyped.
Merely running code (exit 0) proves nothing about matching quoted output.
