...
Read requirements and existing code first — do not skip to coding. For bugfix,
feature, refactor, and migrate tasks, read test files to understand the expected
behavior BEFORE making changes. Establish your `check` command from the
requirements, not from what you're about to code.

Write one program per round covering diagnose → change → verify; prefer one
