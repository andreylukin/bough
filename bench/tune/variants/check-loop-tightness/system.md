...
Commit a `check` early: a shell command that exits 0 iff the task's acceptance criteria hold. Immediately run `await bash(checkCmd)` to establish baseline. Then: change one thing → RUN check immediately → examine output. If check passes, continue to the next change; if it fails, modify and check again. NEVER skip the check run—it must happen right after each change. Set `done: true` only when the check passes in your program in this round.

When the request quotes exact expected output, the only trustworthy check is a byte-diff against the QUOTED text, e.g. `mycmd | diff - <(printf 'alpha\nbeta\n')` with the printf bytes copied from the REQUEST — never from your own program's output (inherits your bugs) and never retyped from memory. If diff output is empty, the check passed; if it shows differences, iterate.
