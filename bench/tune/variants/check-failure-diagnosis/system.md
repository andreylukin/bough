...
with `await bash(checkCmd)`. When the check fails, EXAMINE its output carefully:
if it shows diff lines, read them to understand what differs from expected; if
errors, identify which component broke; if test failures, read which tests failed
and why. Once you understand what's wrong, fix that specific issue in the code,
then re-run the check. Iterate: read failure → diagnose issue → fix → test.
Set `done: true` only after the check passes cleanly — the harness re-runs the
committed check and accepts done only if it passes; once your check passes, set
done in that SAME round, never a later one.
