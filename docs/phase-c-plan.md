...
`scripts/audit-plugins.sh` (rewritten), `scripts/tui/27-preview.sh`, `scripts/tui/28-timeline.sh`,
`scripts/tui/29-drift.sh`, `scripts/tui/30-swap-digging.sh`,
...
Unit tests it must ship: `scripts/tui/27-preview.sh` → `the_preview_opens_on_slash_preview`,
`the_header_names_the_agent_and_the_as_of`, `esc_dismisses_and_the_composer_keeps_the_draft`,
`the_pane_scrolls_with_the_wheel`, `at_34_rows_the_preview_costs_nothing_and_the_layout_reflows`;
`28-timeline.sh` → `the_timeline_opens_on_slash_timeline`, `a_typed_filter_narrows_the_rows`,
`two_filters_compose`, `a_bad_filter_word_is_named_and_the_previous_filter_survives`,
`clicking_a_row_focuses_that_agent_and_step`, `esc_clears_the_editor_then_dismisses`,
`the_pane_collapses_below_its_breakpoint`; `29-drift.sh` → `the_dashboard_lists_every_resident`,
`a_flagged_agent_is_marked_and_a_thin_sample_says_so`, `r_twice_dispatches_the_reset`,
`the_reset_notice_names_the_agent`, `esc_disarms_without_resetting`;
`30-swap-digging.sh` → `disabling_the_preview_row_removes_it_and_the_layout_reflows`,
...
| **V6** the drift dashboard renders drift-watch's per-agent signals and the reset command is reachable from it | `scripts/tui/29-drift.sh::{the_dashboard_lists_every_resident, a_flagged_agent_is_marked_and_a_thin_sample_says_so, r_twice_dispatches_the_reset, the_reset_notice_names_the_agent, esc_disarms_without_resetting}`; `plugins/tui-drift/tests/reset_reachable.rs::the_command_the_pane_returns_is_registered_by_drift_watch`; `pane::tests::{r_twice_returns_the_reset_command_for_the_focused_row, the_reset_command_is_exactly_slash_reset_agent}` |
