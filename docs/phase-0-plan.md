...
boot `--profile tui` with `BOUGH_HOME` pointed at a `TempDir`; `quiesce()`; assert
`hello.greeter` is ACTIVE against provider `greeting-echo`. Write
`{ entries: { greeting.provider: { plugin: greeting-shout } } }` into
`$BOUGH_HOME/bough.patch.yml`; wait for the debounced watch and `quiesce()`. Assert: hello's
`FiberUid` changed (it reloaded), the trace shows `greeting-echo:unload` strictly before
...
`FiberUid` is unchanged. No recompile, no restart, in one test process.
