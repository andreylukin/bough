...
2. **Every turn must produce user-visible text.** A turn of nothing but tool calls shows
   a stack of collapsed cards and no answer, so the agent looks mute. Narration counts for
...
The distinction matters. The program sidecar is deliberately uncontained, which is the
