...
| | |
|---|---|
| `await bash(cmd, tags)` | One command in the workspace. Returns **a string**, the combined output. Carries the turn's interrupt. `tags` is required. |
...
Paths passed to the raw runtime must be absolute, because the program's working directory is
