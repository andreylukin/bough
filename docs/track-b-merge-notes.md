# Track B — what this track wants from the crates it may not edit

Track B (`rebuild-b`, Phase 6) adds NEW CRATES AND NEW ROWS ONLY. The crates listed as off-limits in
`docs/phase-6-plan.md` §0 are not edited by any work package. Where a design wanted a hook that does
not exist on one of them, the crate here builds against the public API that DOES exist and the want
is recorded below, with file, signature and reason, for the merge agent.

These are WANTS, not blockers: everything in `docs/phase-6-plan.md` is implementable without them.

---

## 1. `plugins/worker-spawn/src/boundary.rs` — compose the block instead of restating it

```rust
pub const WRITE_BOUNDARY: &str = concat!(
    WORKER_PREAMBLE,
    bough_plugin_boundary_instructions::BOUNDARY_BLOCK,
    REPORT_INSTRUCTIONS,
);
```

**Why.** V3 asks that the resident's projection and the spawned worker's request carry the same
bytes from the same source. That half IS proven on this branch, on the requests the adapter
actually received, by
`crates/bough/tests/boundary_injection.rs::the_boundary_block_reaches_the_adapter_on_all_three_paths_with_identical_bytes`:
the `boundary` row's section is `SectionScope::Global`, so BOTH a resident wake's request and a
spawned worker's request carry `BOUNDARY_BLOCK` byte for byte in the system prefix.

What is NOT folded is `worker-spawn`'s own `WRITE_BOUNDARY`, a SECOND, worker-framed block the
spawner prepends to the task message on top of the projection (P6-D3). `plugins/worker-spawn` is
off-limits to track B, so this concat is the merge agent's to make.

**Keep the narrowing when you fold.** The two texts are not interchangeable and the difference is
deliberate: `BOUNDARY_BLOCK` SANCTIONS the four outward acts for an agent, while `WRITE_BOUNDARY`
REFUSES all four to a worker ("You may NOT act outward … they belong to the agent that started
you"). A naive concat that drops `WORKER_PREAMBLE` would silently widen every worker's authority.
Three tests in `plugins/boundary-instructions/src/lib.rs` guard the fold in both directions and
must stay green through it:

- `both_statements_of_the_boundary_name_all_four_sanctioned_acts` — reads the
  `SANCTIONED_ACTS` table, which carries each act's spelling in EACH text (they word the same act
  differently: "push to a pull request" vs "updating a pull request", so one shared substring
  would have pinned nothing).
- `the_spawner_block_refuses_to_a_worker_what_the_boundary_sanctions_for_an_agent` — the narrowing.
- `both_statements_demand_a_citation`.

## 2. `plugins/actions/src/lib.rs` — `find_marker` on `ActionProvider`

```rust
#[async_trait::async_trait]
pub trait ActionProvider: Send + Sync + 'static {
    // …
    async fn find_marker(
        &self,
        kind: ActionKind,
        canonical_target: &str,
        marker: &str,
    ) -> Result<Option<ActionArtifact>, ActionError> { Ok(None) }
}

impl ActionsHandle {
    pub async fn reconcile(&self, now: DateTime<Utc>) -> Result<ReconcileReport, ActionError>;
}
```

**Why.** §7 makes reconciliation a lookup against the world through the provider. `plugins/actions`
is off-limits here, so `plugins/actions-reconcile` declares a SECOND trait (`ArtifactLookup`) and
owns its own registry under the `action_lookup` key (P6-D12). With the method on `ActionProvider`,
`actions-reconcile` folds away and the second registry disappears.

## 3. `plugins/actions/src/lib.rs` — `execute_by_name`

```rust
impl ActionsHandle {
    pub async fn execute_by_name(&self, kind: &str, req: ActionRequest) -> Result<ActionArtifact, ActionError>;
}
```

**Why.** V10 asks for an unspellable action kind to be refused BY THE EXECUTOR. Today
`runtime_actions::parse_kind` refuses it one step earlier, and only a spellable-but-unprovided kind
reaches `ActionError::NoProvider`. Both refusals are tested; only the second is literally "by the
actions executor".

## 4. `plugins/actions/src/error.rs` — `ActionError::BadPayload`

```rust
#[error("`{kind}` payload is not what §7 sanctions: {detail}")]
BadPayload { kind: &'static str, detail: String },
```

**Why.** `actions-linear` refuses a `linear_write` payload naming a title, a team or a new issue —
that is how "ticket creation stays Andrey's" is enforced in the provider as well as by the absent
kind. With no `BadPayload` variant, that refusal has to surface as
`ActionError::Provider { source: LinearActionError::BadPayload }`, which reads as a provider
malfunction rather than a refusal.

## 5. `plugins/actions/src/journal.rs` — an `idem_key` filter on the journal lookup

**Why.** `row_with_idem_key` currently scans every row; its own comment says so. Phase 6 adds a
reconciliation pass that calls it once per pending row.

## 6. `plugins/tools/src/tool.rs` — carry a `StepId` on `ToolCall`

**Why.** Named as a Phase 2 deviation already. Every Phase 6 tool that reaches `ctx.actions`
inherits the synthesised `"{wake}#{step_index}"` triggering step, and with it the consequence that
two calls to one target inside one step collide as `Duplicate`.

## 7. `plugins/agents/src/mail.rs` — `Sender::Ward(String)` and `Sender::Hook(String)`

**Why.** Runtime code posts into a lane's own chat as `Sender::System("ward:<name>")`, which interns
a leaked `&'static str` per distinct ward name.
`bough_plugin_runtime_actions::RuntimeSource::sender_label` is where the spelling lives today.

## 8. `plugins/projection/src/section.rs` — `SectionScope::Kind(AgentKind)`

**Why.** Not needed for V3 (`SectionScope::Global` reaches residents and workers alike, which is
exactly what makes V3 provable). Wanted for anything that should differ between them.

## 9. `plugins/tui-shell/src/pane.rs` — the deferred-work outcome named in `docs/phase-3-plan.md` §6.2

**Why.** The drafts pane reads the ledger in `handle` and inherits the same event-loop blocking.

---

## Deviations this track took, for the merge agent to know about

- **`RuntimeAction::Act`'s `kind` field is spelled `action_kind` ON THE WIRE.** The enum is
  `#[serde(tag = "kind")]` and serde refuses a variant field that shadows its own internal tag. The
  Rust field name is still `kind`.
- **`plugins/actions-reconcile` owns an `action_lookup` service key.** It disappears with merge
  note 2.
- **WP-3: `plugins/actions/src/error.rs` — `ActionError::BadPayload { kind, detail }`.** A
  Provider's payload refusal ("both `status` and `comment`", "no commits to push") is a BAD PAYLOAD,
  not a provider malfunction, and today it can only surface as `ActionError::Provider { source }`
  wrapping `GhActionError` / `LinearActionError`. The wrapping is what the WP-3 tests assert on;
  with the variant they would read the kind directly.
- **WP-3: `actions-github` and `actions-linear` call `gh` / Linear through their own narrow
  transport traits** (`GhRunner` in `plugins/actions-github/src/runner.rs`, `LinearApi` in
  `plugins/actions-linear/src/lib.rs`), with `GhCli` / `LinearHttp` as the one production
  implementation each. That is what lets the tests inject a recording fake with no `PATH` shim and
  no HTTP stub; nothing outward-facing is reachable from a test.
- **WP-3: `push_to_pr` is a fast-forward of the PR's head ref through `gh api`
  (`PATCH repos/{o}/{r}/git/refs/heads/{head}`), and it REFUSES a push whose head commit does not
  already carry the `Bough-Action:` trailer.** §2.5 puts `push_to_pr`'s marker in the commit
  message, which only the process that MADE the commit can do; the Provider therefore verifies the
  trailer by reading the commit before it moves anything, rather than pushing an artifact that
  reconciliation could never find. `marker::commit_trailer` is the pure helper the committing side
  uses.
- **WP-3: `bot_thread_op` always leaves a comment, then resolves/closes.** A resolve with no
  comment would leave no marker in the world and could not be reconciled after a crash.
- **WP-3: `actions-reconcile` reaches drafts through its own `Drafting` trait**, implemented for
  `DraftsHandle`. It exists so the reconciliation pass is testable without the `drafts` row; it
  should collapse onto `DraftsHandle` at merge.
- **WP-3: `GithubActions::open` does not resolve `me` at activation** (the scaffold said it did).
  `gh api user` is resolved lazily and cached on first use, so a row cannot make boot fail over a
  network that nothing has asked it to reach yet. `GithubActions::check_bot_thread` also takes the
  repo as a parameter: a review comment id is only addressable under its repo.

## 10. `crates/bough-kernel/src/context.rs` — dispose ONE child a plugin mounted

```rust
impl Context {
    /// Mount `entry` as a child of this fiber, returning a handle that DISPOSES that child.
    pub async fn mount(&self, entry: Entry) -> Result<FiberHandle, KernelError>;
}

impl FiberHandle {
    pub async fn dispose(&self);
}
```

**Why.** Both file-watching hosts reload by disposing exactly one child and remounting it
(`plugins/skills`, `plugins/wards-rhai`). `Context::mount` returns a `FiberHandle` with no disposal,
and the cascade lives on the fiber's parent uid rather than on the parent's effect accumulator, so
the only way in is `ctx.kernel().unwrap().runtime().dispose(uid)` —
`plugins/skills/src/lib.rs::reconcile` does exactly that. It works and it is public, but it reaches
through `Kernel` and `FiberRuntime` for something a plugin should be able to say on its own handle.

## 11. `plugins/hooks-exec` — two harness points are declared and not wired ✅ CLOSED

**Closed by the review-fix pass.** Both events are defined by track-B crates on this branch
(`bough_plugin_schedule::ScheduleFired`, `bough_plugin_power::PowerChanged`), so the two listeners
are wired here and no merge action is needed. Their `hook/fired` rows land on the `system`
trajectory with a synthetic trigger, because a job fire and a power change belong to no agent's
trajectory. `HooksConfig` also gained a point-name check in two halves: `is_point_shaped` at load
(self-contained) and the `every_configured_point_is_a_point_that_exists` invariant at quiesce (the
step-type vocabulary is not complete until the tree is up). Proven by
`crates/bough/tests/hooks_journal.rs::{the_power_changed_harness_point_fires_on_a_real_power_event,
the_schedule_fired_harness_point_fires_on_a_real_job_run}`.

## 12. `plugins/agents/src/agent.rs` — a way to know when a requested wake FINISHED

```rust
impl Agent {
    /// The wake `request_wake` opened, awaited to completion.
    pub async fn when_wake_done(&self, wake: &WakeId);
}
```

**Why.** `catch-up-on-wake` must drop a second `DidWake` that arrives while the first catch-up is
still running (§2.14). "Still running" is the window between `WakeRequest::Started(id)` and that
wake ending, and the seam exposes no such await: the row uses `Agent::when_idle()` on a spawned
task instead, which is the right answer for an agent doing nothing else and a slightly early one
for an agent that was already mid-wake when the machine woke. The `in_flight` set is the row's own
(`CatchUpOnWake::finish`), so the tests are deterministic either way.

---

## Deviations WP-8 took, for the merge agent to know about

- **`plugins/power` exposes `dispatch(&Context, PowerEvent)` and every Provider goes through it.**
  The Definition's invariant is "the mounted source's `last()` is the last payload that went
  through the seam", and a Provider that dispatched on its own could not be held to it. The
  Definition therefore declares `Inject::optional(["power"])` — a Definition reading the key it
  defines, for the invariant and nothing else.
- **`sleep-listener` owns a `Gate`**, the platform-independent half of a source: it remembers the
  `WillSleep`, measures the wake, drops a wake under `min_sleep_ms`, and writes `last` BEFORE
  dispatching. A dropped wake never becomes `last()`, which is what keeps the seam's invariant
  true. Both macOS hooks and the no-op source are wrapped in one `GatedSource`.
- **`sleep-listener` DEGRADES rather than refuses when `source: auto` and IOKit gives no port**:
  IOKit → NSWorkspace → `noop`, with a warning at each step. An EXPLICIT `iokit`/`nsworkspace` off
  macOS is refused loudly by `choose`, and an explicit `nsworkspace` that will not start on macOS
  fails the row. §13 makes TUI-launch catch-up the reliable baseline, so a laptop that refuses to
  boot bough over a power notification is the worse outcome.
- **The NSWorkspace fallback is hand-rolled through the Objective-C runtime** — `objc_getClass`,
  `objc_allocateClassPair`, `class_addMethod`, and `objc_msgSend` CAST TO ITS EXACT SIGNATURE at
  every call site (the variadic declaration is wrong on aarch64). No `objc2`/`cocoa` dependency was
  added. `NsWorkspaceSource::post` exists so the observer can be exercised by posting into the
  workspace's own notification center, which is how
  `plugins/sleep-listener/tests/macos_ffi.rs::the_nsworkspace_observer_receives_a_posted_sleep_and_wake`
  proves the path without a real sleep.
- **`env("LINEAR_API_KEY")` in the two Linear rows became `env_or("LINEAR_API_KEY", "")`.** As
  written in `docs/phase-6-plan.md` §2.18 it made EVERY boot on a machine without the key fail
  composition with `MissingEnv` — `--dump-config`, `--check`, `make tui-test` and the audit
  included. Both rows already treat an empty key as "this source is off" and say so in a warning.
- **`mcp.call` and `wards.test` are in `bundles/bough-headless.yml` only**, not in
  `bough-base.yml`. §2.18 lists them in the base block and then says headless gains them; headless
  includes base, both subcommands force the headless profile, and putting them in base would mount
  two inert CLI rows in the TUI as well.
- **The subcommand's profile is read, not written.** `Cli::effective_profile()` returns the
  subcommand's profile (headless for all three) and `compose::plan_layers` resolves through it, so
  `--dump-config` and boot agree without either having to normalize first. `Cli::normalize()`
  (called from `main`) only sets `no_watch`. `exec::force_profile` is untouched.

### What WP-8 leaves for the merge

- `plugins/hooks-exec`'s `power/changed` harness point (merge note 11) can now be wired: the event
  is `bough_plugin_power::PowerChanged`, a `ParallelEvent` with payload
  `bough_plugin_power::PowerEvent`, subscribed with `ctx.on_parallel::<PowerChanged, _, _>`.
- The `wards` and `skills` fibers do not settle within the kernel's 5s teardown budget on
  `--check` (`fiber did not settle within 5s; disposing it anyway`). The tree still exits 0 and no
  invariant fires, but the file watchers those two rows hold are not being stopped by their own
  disposers. WP-6/WP-7's crates own it.

## 15. Integration pass (track B, all eight work packages together)

Four things only showed up once every WP's crates were in one tree and `make gates` ran end to end.
All four are fixed on this branch; they are recorded because three of them are notes an earlier
section got wrong, and the fourth is a shape the merge should keep.

- **`actions-github`, `actions-linear` and `actions-reconcile` now declare `ledger` in `inject`.**
  Each one's runtime invariant folds its own `ActionRow`s off `ctx.get::<Ledger>()`, and the
  kernel's undeclared-read check reported all three on every scripted session
  (`crates/bough/tests/agent_invariants.rs::every_invariant_reports_clean_over_a_scripted_session`).
  `required`, not `optional`: the `actions` seam these Providers already require itself requires
  `ledger`, so the declaration cannot widen a tree that would otherwise boot.

- **WP-8's closing note — "the `wards` and `skills` fibers do not settle within the kernel's 5s
  teardown budget" — was a real bug in those two crates, not a watcher-disposal gap, and it is
  fixed.** Both hot-reload tasks sat in `while rx.recv().await.is_some()`, which never reaches a
  halt checkpoint; `EffectInner` awaits a spawned body before running its inverses, so the fiber
  could not settle and the kernel timed it out three times over (`wards`, `skills`, and `commands`
  waiting on them as their provider). Both now poll the channel on a 100ms `RELOAD_POLL` timeout
  and read `EffectCtx::is_halted` between polls. `bough --profile tui` SIGINTs to a clean exit in
  under a second with no "did not settle" line. **Any other long-lived `effect_spawn` body added on
  the merge branch owes the same shape**: an awaited receive with no checkpoint is a teardown hang,
  not a warning.

- **`disabled: true` on the old-feed row retires two Phase 3 surfaces, so those tests re-enable
  exactly that row by patch.** `crates/bough/tests/old_feed_surface.rs` (via the new fixture
  `crates/bough/tests/fixtures/old-feed-on.yml`) and `scripts/tui/07-old-feed.sh` are about the
  ADAPTER, not about whether the shipped tree still boots it — and flipping `disabled` back is
  itself the live proof that the one-week revert path works. `tui_swap.rs` instead asserts the row
  is now `Inactive` in the shipped tree.

- **`tui_swap.rs`'s reflow assertion was racing the next frame.** `TuiHandle::rect_of` answers from
  the LAST DRAW's rectangles while unregistering a pane drops its rect immediately, so membership
  updates synchronously and geometry only catches up on the following frame. Adding `tui.drafts` to
  the bundle changed the timing enough to expose it. The assertion now waits (bounded, 5s) for the
  reflow, and asks whether SOME surviving pane grew rather than naming `tui.focus` — which pane
  absorbs the freed rows depends on who else is mounted in the slot.


## 16. The first message after a cold boot can be swallowed (found by V4's probe)

`scripts/tui/27-drafts.sh` was intermittently failing (~1 run in 4, always the first run of a batch,
always on a cold `$BOUGH_HOME`): after `tui_start` returned and the screen already showed the agent
row, the drafts pane and the composer prompt, the first `shell-use submit` left NO trace at all —
no `user/message` step, no `wake/start`, an empty `ledger.db`. A second submit in the same session
always works. The message is dropped between the composer and the ledger, silently.

The crates that own that path (`tui-shell`, `agents`, `residents`) are ones this track may not edit,
so the script now asserts the echo and retries up to three times
(`the_composer_takes_the_message`), and the drop is recorded here and in `docs/phase-6-plan.md` §6.
**What the merge should do:** find where a composer submit becomes mail, and make the pre-ready case
either queue or report — a submit that reaches no ledger row and no error is indistinguishable, from
the outside, from a boundary refusal, which is exactly the confusion it caused here.

## 17. The ward host bounds its firing loop by RATE; provenance is the better fix

Found by the verification pass (P6-D17 in `docs/phase-6-plan.md`). A ward whose actions cause an
agent to write a step the ward triggers on feeds itself through that agent and fires forever. The
host now bounds it with `WardHostConfig::max_firings_per_minute` (validated `1..=600`, `60` in
`bundles/bough-base.yml`), because a rate catches every loop shape and needs one field.

**What the merge agent should consider instead.** The exact fix is provenance: a ward does not fire
on a step its own actions caused. That needs a cause carried from `runtime-actions`' `RuntimeCx`
(which already knows `RuntimeSource::Ward(name)` and the triggering `Trigger`) through the mail a
`hint` sends, onto the wake it opens, and onto the steps written in that wake — touching
`plugins/agents` and `plugins/workers`, both off-limits to this track. With provenance in place the
rate bound stays as the backstop for loops that close through something else (two wards triggering
each other, a hook, a schedule), so this is an ADDITION, not a replacement.

Merge note 7 (`Sender::Ward(String)`) is the first half of that plumbing and should land first.


---

## 18. What the review-fix pass changed that the merge should know about

- **`plugins/actions-github`'s `bot_thread_op` payload changed shape.** `thread: String` is now
  `comment_id: ReviewCommentId` (a branded u64 — the REST review-comment database id). The
  GraphQL ids the resolve and close mutations need are LOOKED UP (`GithubActions::thread_node_id`)
  and never spelled by the caller. `close` is now `minimizeComment`, a different mutation from
  `resolve`'s `resolveReviewThread`. Anything on the merge branch that constructs a
  `bot_thread_op` payload must be updated.
- **`bundles/bough-base.yml`: `skills.dir` and `wards.dir` are `bough_path(...)`, not
  `home_path(".bough/...")`.** Same location for a real user; hermetic for any test that isolates
  only `$BOUGH_HOME`. Any other row whose default should follow bough's home rather than the
  user's home wants the same treatment — `old-feed`'s `bough_db`/`jungler_db` are deliberately
  `home_path`, because they name the OLD installation's files.
- **`schedule-cron`'s `tick_ms` config field is GONE**, replaced by
  `bough_plugin_schedule_cron::SCHEDULER_TICK_MS`. A bundle on the merge branch that still sets it
  will be refused by `deny_unknown_fields`.
- **`mcp-subprocess`'s `ProcessRow` gained `call_timeout_ms` and `boot_timeout_ms`**, both
  validated as non-zero. A `processes:` entry written before this pass will not deserialize.
- **`plugins/wards-rhai` publishes a `wards.config` service key** (the mounted host row's
  `Arc<WardHostConfig>`) so `bough wards test` dry-fires under the deployment's own limits. The CLI
  row declares it OPTIONAL, which makes the row a dependent and therefore re-appliable — hence
  `DRY_RUN_DONE`, which keeps the dry run to once per process.
- **`plugins/collector-linear` exposes `hold_key`/`release_key`.** The row's disposer releases;
  the set is refcounted so two rows on one key do not blind each other's invariant.
- **`plugins/wards-rhai::evaluate` and `::dry_run` take a `Duration` budget.** Any caller on the
  merge branch needs the extra argument.

---

# Merge outcome

Written by the merge of `rebuild-b` into `rebuild` (Phase 6 + Phase 7 hosts joining Phases 4, 5 and
ux1). Every numbered note above is resolved here: **IMPLEMENTED** names the crate and the test, or
**DEFERRED** gives the reason. Nothing is left silent.

Track B kept its brief: no crate on the off-limits list of `docs/phase-6-plan.md` §0 was edited by
it. The only overlap the merge had to resolve by hand was six files — `BUILD.md`, `Cargo.lock`,
`Cargo.toml`, `bundles/bough-base.yml`, `bundles/bough-tui-app.yml`, `crates/bough/Cargo.toml` and
`crates/bough/tests/tui_swap.rs` — all of them union-shaped except the last, which both sides had
rewritten for the same reason and where track B's version (any surviving pane grew, bounded wait)
is the one that survives, with Phase 5's explicit `redraw()` folded into its poll.

## 1. `worker-spawn::WRITE_BOUNDARY` — DEFERRED

The fold as written would make every worker's prompt carry `BOUNDARY_BLOCK` **twice**. The note's
own goal — "the resident's projection and the spawned worker's request carry the same bytes from
the same source" — is already met and proved, on the requests the adapter received, by
`crates/bough/tests/boundary_injection.rs`: the `boundary` row's section is `SectionScope::Global`,
so a spawned worker's SYSTEM PREFIX already contains `BOUNDARY_BLOCK` byte for byte.
`WRITE_BOUNDARY` is prepended to the TASK MESSAGE on top of that. Concatenating the block into it
adds ~1.4 KB of duplicate instruction to every worker request and buys only the deduplication of
source text.

Two smaller facts, for whoever revisits it: `concat!` takes literals, not a `const` from another
crate, so the fold needs `const_format` or a `LazyLock<String>` (a dependency, or a type change at
every call site); and the narrowing the note warns about is real — `WRITE_BOUNDARY` REFUSES to a
worker the four acts `BOUNDARY_BLOCK` SANCTIONS for an agent. The two-way `SANCTIONED_ACTS` pin in
`plugins/boundary-instructions/src/lib.rs` stays as the guard, and all three of its tests are green.

## 2. `ActionProvider::find_marker` and the second registry — IMPLEMENTED

- `plugins/actions/src/lib.rs`: `ActionProvider::find_marker(kind, canonical_target, marker)` with
  a default of `Ok(None)` (a Provider that cannot look its artifact up finds nothing, so the intent
  is SURFACED rather than concluded — the safe direction), plus
  `ActionsHandle::find_marker(...)` routing to the Provider that owns the kind.
- `plugins/actions-reconcile`: `ArtifactLookup`, `ActionLookup` and `LookupRegistry` are **gone**.
  The row is now a pure Consumer of `actions` + `drafts`. `Inject::optional(["action_lookup"])` is
  gone from both Providers, and so is the `actions-github: no `action_lookup` in the tree…` warning
  that `bough --check` printed three times on every boot.
- §18's companion item is done too: the `Drafting` trait collapsed onto `DraftsHandle`, and the
  tests now read the draft back out of the ledger — a stronger assertion than counting trait calls.

`ActionsHandle::reconcile(now)` was deliberately NOT added. The pass needs a draft surface, and
`plugins/actions` is the Service Definition: a Definition that depends on `plugins/drafts` to
conclude its own rows inverts the seam. The pass stays in `actions-reconcile`, which is where a
Consumer belongs.

Proven by `plugins/actions-reconcile/tests/reconcile.rs` (5, incl.
`a_provider_leaves_the_seam_with_its_registration` and `reconciliation_never_calls_a_write_path`,
whose fake Provider now PANICS in `execute` — so a pass that reached a write path fails the test
instead of being observed not to), `plugins/actions-github/tests/reconcile_lookup.rs` (3, the real
`GithubActions` registered on the real seam over a recording `gh`), and
`plugins/actions-linear/tests/writes.rs::find_marker_reads_the_issues_comments_and_writes_nothing`.

## 3. `ActionsHandle::execute_by_name` — IMPLEMENTED (V10's open item 2 CLOSED)

`ActionKind::parse` / `ActionKind::KNOWN` (`plugins/actions/src/kind.rs`),
`ActionError::NoSuchKind { name, known }`, `ActionRequestParts` (an `ActionRequest` whose kind is
still a string) and `ActionsHandle::execute_by_name(ctx, kind, parts)`.
`runtime_actions::parse_kind` is DELETED and `do_act` hands the NAME to the executor, so "there is
no `slack_send`" is the actions seam answering about its own vocabulary rather than a caller's
parser refusing one step earlier. Nothing is journalled for a name that does not exist.
`plugins/runtime-actions/src/lib.rs::tests::a_spelling_outside_the_four_is_refused_by_the_executor`.

## 4. `ActionError::BadPayload` — IMPLEMENTED

The variant exists, and both Providers map onto it in one place each (`gh_error` in
`plugins/actions-github/src/lib.rs`, `linear_error` in `plugins/actions-linear/src/lib.rs`): a
payload §7 does not sanction — a commit with no `Bough-Action:` trailer, both `status` and
`comment`, a ticket title — is a REFUSAL, and only a transport or endpoint failure stays
`Provider { source }`, which is what that variant means. `refusals.rs` and `writes.rs` now assert
the CLASSIFICATION and not just the sentence:
`plugins/actions-github/tests/refusals.rs::push_to_pr_pushes_the_commit_that_carries_the_marker_and_refuses_one_that_does_not`
(`classify(&e) == "bad_payload"`) and
`plugins/actions-linear/tests/writes.rs::a_payload_refusal_is_a_refusal_and_not_a_provider_malfunction`.

## 5. An `idem_key` filter on the journal lookup — IMPLEMENTED

`ActionQuery::idem_key: Option<IdemKey>`, answered identically by `ledger-sqlite` (a `WHERE
idem_key = ?` clause) and `ledger-memory` (the same one-key filter), and used by
`ActionsHandle::row_with_idem_key`. The uniqueness check on every `execute` — and the one
reconciliation makes per pending row — is a one-row lookup instead of a scan of the whole journal.

## 6. A `StepId` on `ToolCall` — DEFERRED

This moves §7's idem key. The key is `sha256(kind ‖ canonical target ‖ triggering step id)` and the
MARKER is derived from it, so carrying a real `StepId` where `"{wake}#{step_index}"` is synthesised
today changes the marker written into every PR body, commit trailer and comment from that point on.
Reconciliation finds a pending row by recomputing its marker, so the change wants a migration note
and a phase of its own, not a merge commit. The consequence stands and is unchanged: two calls to
one target inside one step collide as `Duplicate`. It is a Phase 2 deviation and stays one.

## 7. `Sender::Ward(String)` and `Sender::Hook(String)` — IMPLEMENTED

Both variants exist on `plugins/agents/src/mail.rs` with their wire forms and `as_ref_str`
spellings (`ward:<name>`, `hook:<name>`), and `agent_loop::wake::sender_ref` names them.
`RuntimeSource::sender()` returns them, and `runtime-actions`' private `intern` — which leaked a
`&'static str` per distinct ward file name — is deleted. A subprocess plugin has no variant of its
own and stays `Sender::System("mcp-subprocess")`: the set of process names is fixed by the bundle,
not by a directory anyone can add a file to.

This is also the first half of the provenance plumbing note 17 asks for (see below).

## 8. `SectionScope::Kind(AgentKind)` — DEFERRED

The note says it itself: "Not needed for V3 … Wanted for anything that should differ between
them." Nothing in the merged tree wants a section that differs between residents and workers, and
AGENTS.md's rule is that a plugin gets no surface it does not use. The day something needs it, the
enum grows one variant and `projection-assembler/src/registry.rs` grows one arm.

## 9. `tui-shell`'s pane `handle` does ledger I/O on the event loop — DEFERRED

Already deferred by name in `docs/phase-3-plan.md` §6.2, WITH the reason: fixing it means either a
per-pane work queue or an outcome that says "I will redraw later", and both change the `Pane`
contract — which is §15 item 6's granularity review at Phase 8. `tui-drafts` inherits the
behaviour; it reads a bounded `retain` window and nothing in the suite measures a stall.

## 10. `Context::mount` returning a disposable handle — DEFERRED

Ergonomics, and a kernel capability decision rather than a merge decision. `FiberHandle` is also
what `FiberRuntime::list`/`get` hand back, where "dispose" is emphatically not the caller's to say;
so `FiberHandle::dispose()` is either a method that silently does nothing on most handles (a
footgun) or a method that lets any plugin dispose any fiber it can name (a capability widening that
§0.3 exists to prevent). The narrow fix — a back-reference only `mount` sets — is a kernel design
change and belongs to whoever owns §0.3, not to a merge.

Both file-watching hosts use the public `Kernel::runtime().dispose(uid)` today, and their reload
tests are green: `plugins/wards-rhai/tests/reload.rs` (3), `plugins/skills/tests/watch.rs`.

## 11. `hooks-exec`'s two unwired harness points — CLOSED on `rebuild-b`

No merge action. Both events are defined by track-B crates, both listeners are wired, and
`crates/bough/tests/hooks_journal.rs` proves each on a real event.

## 12. `Agent::when_wake_done` — IMPLEMENTED

`AgentDriver::when_wake_done(&WakeId)` (default: return at once, which is the honest answer for a
driver that keeps no per-wake state), `Agent::when_wake_done`, and `agent-loop`'s override polling
`DriverState::interrupts` — the map that gains an entry when the wake is minted and loses it when
the wake's task ends. `CatchUpOnWake::on_wake` now reports `(AgentId, WakeId)` and the window-closer
waits for THAT wake instead of `when_idle()`, which is §2.14's window exactly: an agent that was
already mid-wake when the machine woke no longer closes somebody else's window.
`plugins/catch-up-on-wake/tests/wake.rs` (6, all green through the change).

## 15. Integration pass — ALL FOUR CARRIED, verified on the merged tree

The note is not a want but a record of four fixes track B made once its eight work packages were in
one tree. All four survive the merge, checked here against the merged code rather than the note:

- `actions-github`, `actions-linear` and `actions-reconcile` each declare `ledger` in `inject`
  (`plugins/actions-github/src/lib.rs:672`, `plugins/actions-linear/src/lib.rs:373`,
  `plugins/actions-reconcile/src/lib.rs:150`), all three `Inject::required` beside `actions`, so the
  kernel's undeclared-read check stays quiet over a scripted session.
- The teardown fix is intact: `RELOAD_POLL` (100ms) plus an `is_halted` check between polls in
  `plugins/skills/src/lib.rs:38,242` and `plugins/wards-rhai/src/lib.rs`. The standing rule it
  states — a long-lived `effect_spawn` body owes a halt checkpoint — is repeated under §18 below and
  holds for everything `speed` and `ux-visual` added since.
- The retired old-feed row is still `disabled` in the shipped tree, and the two surfaces that test
  the ADAPTER still re-enable exactly that row by patch: `crates/bough/tests/fixtures/old-feed-on.yml`,
  `crates/bough/tests/old_feed_surface.rs`, `scripts/tui/07-old-feed.sh`. `tui_swap.rs:63` asserts
  the shipped row is `Inactive`.
- `tui_swap.rs`'s reflow assertion is the bounded-wait, some-pane-grew form (`tui_swap.rs:148-178`),
  with Phase 5's explicit `redraw()` folded into its poll loop, as the merge preamble says.

Nothing here was re-opened by `speed` or `ux-visual`.

## 16. The first message after a cold boot — IMPLEMENTED, and the diagnosis was half wrong

**The drop is real and it is reproducible.** It is NOT in `agent-loop`, and it was never silent.

Measured against the release binary on a cold `$BOUGH_HOME`, submitting as soon as the composer
prompt paints: **8 of 10 runs** lost the message with no delay at all, and **2 of 10** with a 50 ms
pause first. Every single loss had `inbox/spliced insert = 0` **and** `wake/start = 0` — the
message never reached the ledger at all — and every single one had `no focused agent` on screen.
There is no run in which the message reached the ledger and no wake followed, which is what a loop
bug would look like. `agent-loop` never dropped a message it was handed.

What actually happens: `residents` raises the roster asynchronously AFTER the shell mounts, so
`TuiHandle::agent()` is `None` for the first few hundred milliseconds of a cold boot;
`run::send` then reported "no focused agent" and put the text back in the composer. Track B read
that as a silent drop for a good reason — the bounced text is on screen, so a script that waits for
its own message to appear passes either way. That is exactly what note 19 caught in
`27-drafts.sh` from the other end.

The fix is what the note asked for: the pre-ready case **queues**.
`TuiInner::pending_send` holds the submit, the tick that adopts the boot focus also drains it
(`run::flush_pending_send`), and past `PENDING_SEND_TICKS` (10 — a protocol constant) the text is
handed back to the composer with an error, because a tree with genuinely no agents must say so and
nothing the user typed is ever destroyed (B3).

`plugins/tui-shell/tests/pending_send.rs` (3):
`a_message_typed_before_the_roster_is_up_waits_and_lands_when_an_agent_appears` (asserted on the
agent's INBOX, not on a notice), `a_queued_message_comes_back_to_the_composer_when_no_agent_ever_appears`,
`a_second_submit_while_one_is_queued_is_handed_back_rather_than_replacing_it`. All three FAIL on the
pre-merge `send` — verified by reverting it in place and re-running: 0 passed, 3 failed.

`27-drafts.sh`'s `submit_until_echoed` retry is left in place. It is now belt and braces rather
than the only thing standing between the suite and a flake.

## 17. Ward provenance — DEFERRED, and half the plumbing has landed

`WardHostConfig::max_firings_per_minute` stays as the bound. Merge note 7 — the first half the note
names — is done: a ward posts as `Sender::Ward(name)` now, so the cause has a spelling. The rest is
what the note describes: carrying that cause from `runtime-actions`' `RuntimeCx` through the mail a
`hint` sends, onto the wake it opens, and onto the steps written in that wake. That is a new field
on `Message`, on `WakeSpec` and on every step body a wake writes — a step-body schema change, which
§0.2 makes a ledgered, versioned act rather than a merge edit. The rate bound catches every loop
shape today, including the ones provenance would not (two wards triggering each other, a hook, a
schedule), so nothing is unguarded in the meantime.

## §18 (what the review-fix pass changed) — all carried, nothing to undo

- `bot_thread_op`'s `comment_id: ReviewCommentId` payload: nothing on the `rebuild` side constructs
  a `bot_thread_op` payload, so there was nothing to update.
- `skills.dir` / `wards.dir` as `bough_path(...)`, `old-feed`'s `home_path` deliberately left:
  carried verbatim into the merged `bundles/bough-base.yml`.
- `schedule-cron`'s `tick_ms` gone, `mcp-subprocess`'s `call_timeout_ms` / `boot_timeout_ms`,
  `wards.config`, `collector-linear`'s `hold_key`/`release_key`, `wards-rhai::evaluate`/`dry_run`
  taking a `Duration`: all carried; no bundle row or caller on the `rebuild` side used the old
  shapes.
- §15's standing rule — **a long-lived `effect_spawn` body owes a halt checkpoint** — is honoured by
  everything this merge added. The only new long-lived body is `catch-up-on-wake`'s window-closer,
  which was already polled against `is_halted` and still is.

## The collectors now hand their mail to the router

Not a numbered note, but the biggest behavioural change the merge makes, and §0 rule 3's own
promise ("every delivery carries the refs that `mail-router` will later route on") coming due.

- `Envelope` gains `dedupe_on: Option<Ref>` and `RouteReport` gains `deduped: Vec<AgentName>`. The
  AT-LEAST-ONCE guard moved WITH the fan-out, because the router is what chooses recipients and a
  collector handing over an envelope does not know who will get it. `mail_router::already_delivered`
  is the one implementation of the rule; `collect_core::guard::already_delivered` delegates to it
  so the fallback path cannot drift.
- Both collectors declare `Inject::optional(["mail"])` and route through it when it is there. With
  no `mail` seam they fall back to their own `deliver_to` list, exactly as before, and a row with
  NEITHER a router nor a list says so — in the SWEEP, every sweep, with a `disabled` entry and no
  outward call, rather than at activation. At activation it was wrong: `mail` is optional, the
  committed view is captured at apply time, and a row that applies in the window before `mail` is
  bound sees none and then reloads and sees it. The shipped tree printed *no `mail` seam and an
  empty `deliver_to`* on almost every boot about a row that had the seam a moment later.
  `crates/bough/tests/collector_schedule.rs::both_collectors_have_the_mail_seam_in_the_shipped_tree`
  is the assertion that the FINAL view really does carry it.
- `bundles/bough-base.yml` sets `deliver_to: []` on both rows: `mail` is in the same bundle, so a
  lane gets a repository's mail by linking the ref, not by being named in a collector's config.
  `deliver_to` is no longer a validated-non-empty field.
- **Every collected item now carries a SCOPE ref beside its own** — `repo:o/r` for GitHub,
  `team:TEAM` for Linear (`collect_core::refs::{repo, team}`). Without it the router was
  unusable in practice: an item's own ref is `gh:o/r#12`, and nobody links a lane to a pull
  request that does not exist yet. The scope ref is what a person actually links a lane to, it is
  added at PARSE time so both destinations carry it, and it is what
  `crates/bough/tests/phase6_swap.rs` now links `sol` with.
- `plugins/collector-github/tests/routing.rs` (3):
  `with_a_router_the_lane_that_matches_the_ref_gets_the_mail_and_deliver_to_is_not_read`,
  `a_re_offered_item_is_deduped_by_the_router_and_reported_as_skipped` (a lost watermark, which is
  the case the guard exists for), `with_no_router_the_deliver_to_list_is_the_destination`.
  `plugins/collect-core/src/delivery.rs::tests::an_envelope_is_the_same_mail_plus_the_dedupe_key`
  pins that the two destinations carry the same bytes.

## The shell-use suite

Track B's four scripts are renumbered so both sides' numbering survives:
`10-drafts` → `27-drafts`, `11-mcp-tool` → `28-mcp-tool`, `12-swap-collector` → `29-swap-collector`,
`13-swap-wards` → `30-swap-wards`. Both halves glob `scripts/tui/[0-9]*.sh`, so nothing else moved.

## What the merge itself had to fix

Six defects only exist once both sides are in one tree. Each is a real bug with a named test; none
of them is a merge conflict, and none could have been found on either branch alone.

1. **`worker-fork` could not fork in the SHIPPED tree.** Track B left a deliberate tripwire —
   `boundary_injection.rs::no_fork_path_exists_to_assert_on_yet`, a test that FAILS the day a fork
   provider lands, so V3's third path cannot be quietly skipped. Phase 5 landed one, the tripwire
   fired, and writing the arm it demanded found this: `ForkSetup::setup` pinned the child's prefix
   through `prefix::pin(agent.ctx(), …)`, which resolved `Projection` off the CHILD AGENT's
   context — a context owned by the `agents` row, which does not declare `projection` in its
   `inject`. Every `WorkerKind::Fork` in the real bundle therefore died in setup with
   *plugin `agents` (row `agents`) read service `projection` without declaring it*. No test saw
   it: `plugins/worker-fork`'s own fixtures mount their own `agents`. The handle is now PASSED
   into `prefix::pin` from the fork row (which already holds it — it assembled the prefix with
   it), and the effect still belongs to the child, so P5-D12 is unchanged. Proven by
   `crates/bough/tests/boundary_injection.rs::the_boundary_block_reaches_the_adapter_on_all_three_paths_with_identical_bytes`,
   whose third arm asserts the fork STARTS and that the block reaches it verbatim — by a third
   route (a pinned prefix), not the live projection and not the spawner's prepended text.

2. **`power-test`'s `/power` summary said "wake".** Phase ux1's M16 rule
   (`commands::palette::house_word`) refuses a user-facing command summary that uses one of this
   tree's house words, and `wake` is one. `power-test` is the SWAP subject of track B's power gate,
   so `phase6_swap.rs::replacing_sleep_listener_with_power_test_by_patch_keeps_catch_up_working`
   booted it and the row failed. The summary is now plain English; the usage line still spells
   `wake`, because that is the literal the command parses.

3. **`scripts/tui/lib.sh`'s `sql()` turned "the ledger is locked" into "the ledger is empty".** A
   bare `sqlite3` gives up the moment the binary holds a write lock — the ledger's closing
   checkpoint is exactly that — printing `database is locked (5)` on stderr and NOTHING on stdout,
   which `steps_of` reported as a count of zero. The merged tree tears down slowly enough to hit it
   (`01-boot-and-turn.sh::the_turn_landed_as_ledger_steps`). The helper now passes
   `.timeout $SQL_BUSY_MS`: a reader racing a writer waits instead of lying.

4. **`tui_invariants`' two planted violations raced the next paint.** Both recorders are
   LAST-FRAME slots, and both tests planted once and ran the invariants once; a pane repaint
   landing in between wiped the plant, and the runner's silence then read as "the invariant is not
   wired". Both now plant-and-run in a bounded retry (40 × 25 ms). The claim is unchanged — a
   planted violation IS reported — and it still fails if the runner never reports one.

5. **Two boot-time warnings were false on almost every boot**, and a warning nobody can believe is
   worse than no warning. `collector-github` / `collector-linear` announced *no `mail` seam and an
   empty `deliver_to`* from `apply`, before the reload that gave them the seam (moved into the
   sweep — see above); `actions-reconcile` announced *`surface_to` names no live agent* from
   `apply`, before `residents` had raised `sol` (moved into the boot pass, and only when that pass
   actually found unfinished intents, which is the moment a missing referent costs something).
   `bough --profile tui --check` now prints three lines, and all three are true of this machine:
   no Linear key, twice, and `repos: []`.

6. **`phase6_swap.rs` was passing for the wrong reason.** Two of its four gates read `sol`'s
   trajectory after a sweep. With the router in the tree and no lane linked to `repo:o/r`, the
   swept items go to the UNSORTED queue — and reached `sol` only if the leader's adoption pass
   happened to run first, which is why the assertion failed about one run in ten. Both gates now
   link the lane (`link_repo`), which is what the merge made the shipped path. The file's
   `resident()` helper also re-`peek_live`s every poll: a handle captured before a boot-time
   reload of the `agents` row points at a registry that stays empty for ever, which is what
   `live: []` after five seconds of polling turned out to be.

## An environment hazard the gates depend on, recorded because it cost a day

`make test` will fail — every `boot_real` test at once, with `the shipped tree must quiesce` —
whenever `target/debug/deps` has grown to roughly a million files.

The chain: macOS's default `split-debuginfo = "unpacked"` leaves one `.o` per codegen unit in
`target/debug/deps` and cargo never collects them (this worktree had **941,900** of them, 148 GB).
The first `reqwest::ClientBuilder::build()` in a process — `collector-linear`'s, at row activation —
calls `Matcher::system()` → `SCDynamicStoreCreateWithOptions` → `CFBundleGetMainBundle`, and
`CFBundleGetMainBundle` **readdirs the directory the executable sits in**. For a test binary that
directory is `target/debug/deps`, so the call took ~40 seconds, synchronously, on the runtime
thread — and under `#[tokio::test]`'s single-threaded runtime that starves every fiber, so the
kernel's 10 s quiesce ceiling expires with the tree still converging. `--check` on the release
binary never sees it: `target/release/bough` sits in an ordinary directory.

It is not a product bug and there is nothing to fix in the tree; the shipped binary does not live
in a directory with a million files. It IS a standing hazard for the gates, and the cure is
`find target/debug/deps -maxdepth 1 -name '*.o' -delete` (safe: cargo's fingerprints do not
mention those files, so nothing rebuilds — only debugger symbol detail is lost). Whoever owns
build profiles should decide whether `[profile.dev]` wants `debug = "line-tables-only"` or a
periodic sweep; a merge is not the place to change how everyone's debug builds work.

## The `speed` branch — merged AFTER this, and the fold is done

When the track-B merge was written, `speed` had not landed: this tree had no nextest, no
`tests/main.rs`, no `autotests = false`, no `scripts/check-test-mods.sh` and no `make gate-crate`,
so none of the `tests/*.rs` files track B added needed a `mod` line. `speed` (e92a4bc2) and
`ux-visual` (be89c128) have since merged into `rebuild`, and the fold this section predicted was
done in 057ebe05 + 32654e5a: every one of track B's `crates/bough/tests/*.rs` and `plugins/*/tests/*.rs`
files now has its `mod` line in its crate's `tests/main.rs`. Verified post-merge:
`scripts/fold-tests.py` folds 0 crates (nothing is loose) and `scripts/check-test-mods.sh` passes,
as does `make lint` and `cargo check --workspace --all-targets`. Both profiles compose with
`warnings: []` (`bough --profile tui|headless --dump-config`).
