...
`scripts/tui/10-drafts.sh`, `scripts/tui/11-mcp-tool.sh`, `scripts/tui/12-swap-collector.sh`,
`scripts/tui/13-swap-wards.sh`, `docs/track-b-merge-notes.md`.
...
- `scripts/tui/11-mcp-tool.sh` — shell-use: an mcp tool call renders in the focus pane, expands on
...
- `scripts/tui/12-swap-collector.sh` and
...
- `scripts/tui/13-swap-wards.sh` and
...
- **`worker-fork`.** Not on this branch; V3's fork arm is untested and not claimed.
