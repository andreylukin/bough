...
`plugins/tui-focus/tests/program.rs`, `scripts/tui/31-program.sh`.
...
under a narrow terminal. `scripts/tui/31-program.sh` (shell-use, both consumers via
...
`scripts/tui/32-codemode-swap.sh`, `docs/codemode-merge-notes.md`, `docs/phase-codemode-plan.md`
...
- `scripts/tui/31-program.sh` — bullets `program_row_is_collapsed_by_default`,
...
- `scripts/tui/32-codemode-swap.sh` — bullets `typed_rows_before_the_patch`,
...
arm of `scripts/tui/31-program.sh` "has never run". All of that is false at HEAD,
...
- **WP-7 the TUI program row** — `plugins/tui-focus/src/program.rs`, `rows.rs`, `expand.rs`: a
  ...
  `scripts/tui/31-program.sh` runs inside `make gates` (`Makefile`, the
...
- `ms` is read from the closing `tool/result`'s `value.ms` and `ops` only from `program/error`;
  ...
  `scripts/tui/31-program.sh` still asserts only `2 calls`, so the TUI half of the duration is
