...
Four surfaces, in order of how much they are allowed to do.
...
Drop files in `~/.bough/extensions` or `.agents/extensions` — `*.js`, `*.mjs`, `*.cjs`,
...
already have configured in a project. `guard-destructive.lua` and `redact-secrets.lua`
ship **off**. Yours go in `~/.bough/hooks`. `^x` toggles them live.
