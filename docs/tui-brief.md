...
owe" chip: `? question` on the status line (`rows::owed` from the transcript's rows, reported
each frame; dropped last but for the stop key and the product) and `?` after the lane's name on
the rail (`rail::pending_question` from the ledger by name, re-read when a wake ends). The
question rule is a heuristic, no model call: the last message ends with `?`, the turn is over,
nothing from Andrey followed. A lane with no work yet is dim; the leader never is. Not done:
clicking the chip to jump to the question. (The `◇N` open-claims half of this chip left with the
claims demolition, 2026-08-30 — `docs/leader-lanes.md`.)
