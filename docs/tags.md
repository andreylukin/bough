...
— they run commands in a real checkout and benefit from the same vocabulary.
...
- **Directory profiles use plain popularity**, not idf — the question is "what has been done
...
A bare word is treated as `show TAG` — it is the commonest thing to type and the likeliest
...
- **Normalization is pure and total.** `parseTagsArgs` and `normalizeTags` never throw, and
  `runTags` returns an exit code without touching a real process — the whole command is
