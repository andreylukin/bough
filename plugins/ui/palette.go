...
// the name — "mnr" finds "monarch"), each tier alphabetical. The
// order depends on the NAME alone, never on registration order or the
// query length, so a growing query only ever REMOVES rows: a row may
// DEMOTE a tier as the query grows ("dr" is a prefix of "drift" but
// "drf" only a subsequence), which keeps it on screen rather than
// dropping it, and the row under the cursor is never swapped out.
...
// paletteRow renders one row, clipped to width.
func paletteRow(it paletteItem, sel bool, width, col int, th theme) string {
	...
	return head + "  " + th["dim"].Render(clipRunes(it.summary, left))
}
