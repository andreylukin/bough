...
impl CatchUpOnWake {
    ...
    /// Returns whom it woke AND the wake each one opened, so the caller can wait for exactly
    /// that wake to finish (merge note 12) and the test asserts on the set rather than a count.
    pub async fn on_wake(&self, ev: &PowerEvent) -> Vec<(AgentId, bough_plugin_ledger::WakeId)> {
        if !asks_for_catch_up(ev, self.cfg.min_sleep_ms) {
            return Vec::new();
        }
        let mut woken: Vec<(AgentId, bough_plugin_ledger::WakeId)> = Vec::new();
        for agent in self.agents.list() {
            ...
            match req {
                ...
                WakeRequest::Started(wake) => woken.push((agent.id().clone(), wake)),
            }
        }
        woken
    }
...
pub async fn listen(
    ...
    ctx.on_parallel::<PowerChanged, _, _>(move |ev| {
        ...
        async move {
            ...
            for (id, wake) in state.on_wake(&ev).await {
                ...
                let handle = owner.effect_spawn(move |e| {
                    ...
                    async move {
                        ...
                        if let Some(agent) = state.agents().get(&id) {
                            while !e.is_halted() {
                                tokio::select! {
                                    // MERGE (note 12): THIS wake, not "the agent is idle".
                                    // `when_idle` is the right answer for an agent doing nothing
                                    // else and a slightly early one for an agent that was already
                                    // mid-wake when the machine woke — §2.14's window is between
                                    // `WakeRequest::Started(id)` and that wake ending, and this
                                    // is now the thing that says so.
                                    _ = agent.when_wake_done(&wake) => break,
