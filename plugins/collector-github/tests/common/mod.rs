...
use bough_plugin_mail_router::{MailConfig, MailHandle};
...
impl Fx {
    ...
    /// A `mail-router` seam over the same ledger and roster — what a shipped tree gives the
    /// collector through `ctx.mail`.
    pub fn mail(&self) -> MailHandle {
        // In a tree the `mail` row declares these; the fixture mounts no rows, so it declares
        // them the way the row would.
        for def in bough_plugin_mail_router::vocabulary::step_types() {
            let _ = self.ledger.0.register_step_type(def);
        }
        MailHandle::new(
            self.ctx.clone(),
            self.ledger.clone(),
    ...
    /// The collector ROUTING through `mail-router` rather than through its own `deliver_to`.
    pub fn collector_routing(
        &self,
        cfg: GithubCollectorConfig,
        mail: MailHandle,
    ) -> GithubCollector {
        self.collector(cfg).with_mail(mail)
    }
