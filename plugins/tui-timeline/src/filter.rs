...
impl Filter {
    ...
    /// What the pane's header prints. Absolute instants, because a `Filter` carries no clock and
    /// this crate never reads one (deviation D-C-WP2-1: the plan's example printed `since:2h`).
    ...
    /// The words this filter is spelled with, in a fixed order.
...
pub fn render_filter(f: &Filter, _now: DateTime<Utc>) -> String {
    f.terms().join(" ")
}
...
mod tests {
    ...
    fn an_empty_filter_matches_everything() {
        ...
        assert_eq!(f.describe(), "everything");
    }
