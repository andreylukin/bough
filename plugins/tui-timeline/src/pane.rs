...
pub struct TimelineState {
    ...
    /// When the live filter was SET. The header spells `since`/`until` against it, so the header
    /// and the editor line right above it say the same words (D-C-WP2-1).
    pub filter_at: Option<DateTime<Utc>>,
}
...
fn header_of(state: &TimelineState, rows: usize, cols: u16) -> String {
    let mut text = format!(
        ...
        state
            .filter
            .describe(state.filter_at.unwrap_or_else(Utc::now))
    );
    if state.windowed {
