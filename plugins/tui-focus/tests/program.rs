...
/// The collapsed row is ONE line, and it is EXACTLY the pane's width at every width — the
/// guarantee `tool_header` gives every other row, which is what stops a program row from jittering
/// against its neighbours on a narrow terminal.
#[test]
