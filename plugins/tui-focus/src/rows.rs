...
pub fn rows_from_steps(steps: &[Step]) -> Vec<Row> {
    ...
    for step in steps {
        ...
        match kind {
            ...
            // A `draft_*` call is NOT hidden behind its card any more (round 10): under the
            // code-mode default the draft tools are not always reachable, and a call that
            // produced no `draft/*` step then left nothing on screen at all — a draft attempt
            // must never be invisible. The card, when it comes, follows the call row.
