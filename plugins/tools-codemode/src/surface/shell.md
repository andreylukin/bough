...
bough-plugin-tui-focus", "cargo:test:focus"). Tags index the command in your
cross-session history, and a future session finds this command BY these words — so
a bare tool name is a wasted tag: not "wc" but "app:linecount", not "find" but
...
Add one whenever you know what ticket, PR or issue the work is for. Dashes survive
inside a reference and nowhere else, so write linear.eng-1234, not ENG-1234
(which becomes the two useless tags "eng" and "1234"):
...
the same round, not extra turns.
    ...
    await bash("cargo fmt && cargo test -p x && git status", "verify:all");
