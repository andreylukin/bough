...
/// sources for each spelling. A fixture that named a tool no row registers (main's `sh`, or an
/// action kind with no Provider) would pin the surface against a page no agent is ever shown —
...
/// The config `bundles/bough-codemode.yml` actually ships, deserialised.
///
...
fn bundle_config() -> CodemodeConfig {
    let path =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../bundles/bough-codemode.yml");
    let text = std::fs::read_to_string(&path).expect("the code-mode bundle is in the tree");
...
fn the_roster_names_only_tools_this_tree_registers() {
    ...
    // The guard is not vacuous: main's `sh` is exactly the name that used to be here.
    assert!(
        !sources.contains("ToolName::new(\"sh\")") && !sources.contains("TOOL_NAME: &str = \"sh\""),
        "if a `sh` tool ever lands, put it back in REGISTERED and re-record the snapshot"
    );
}
