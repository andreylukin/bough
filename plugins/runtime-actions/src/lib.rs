...
//!
//! Two distinct refusals, and the map (V10) names which is which. MERGE (note 3): BOTH now come
//! from the executor, through `ActionsHandle::execute_by_name` — this crate no longer refuses a
//! name on the seam's behalf.
//!   - `kind` is not one of §7's four ⇒ `ActionError::NoSuchKind`, from the executor.
//!   - it is, but no Provider registered it ⇒ `ActionError::NoProvider`, from the executor.
//!
...
use bough_plugin_actions::{ActionRequestParts, ActionTarget, ActionsHandle};
...
impl RuntimeSource {
    /// The [`Sender`] a post or a hint from this source is sent as.
    ///
    /// MERGE (note 7): `Sender::Ward(String)` / `Sender::Hook(String)` exist now, so a ward or a
    /// hook is named as itself and nothing leaks a `&'static str` per distinct file name. A
    /// subprocess plugin has no variant of its own and stays `System("process:<name>")` — the set
    /// of process names is fixed by the bundle, not by a directory.
    pub fn sender(&self) -> Sender {
        match self {
            RuntimeSource::Ward(n) => Sender::Ward(n.clone()),
            RuntimeSource::Hook(n) => Sender::Hook(n.clone()),
            RuntimeSource::Process(_) => Sender::System(PROCESS_TAG),
        }
    }

    /// The `ward:<name>` / `hook:<name>` / `process:<name>` spelling this source is NAMED by in a
    /// refusal, a journal entry and a synthetic trigger.
...
/// The one interned tag a subprocess plugin posts under. A protocol constant: `Sender::System`
/// takes a `&'static str`, and this crate never leaks one per name (merge note 7).
const PROCESS_TAG: &str = "mcp-subprocess";
...
async fn do_act(
    ...
    // MERGE (note 3, V10): the NAME goes to the executor, not an `ActionKind`. A kind outside
    // §7's four is refused by the actions seam as `NoSuchKind`, which is what makes "there is no
    // `slack_send`" the executor's answer about its own vocabulary.
    let parts = ActionRequestParts {
    ...
    match cx.actions.execute_by_name(&cx.ctx, kind, parts).await {
        Ok(a) => ActionOutcome::Did {
            detail: format!("{kind} {target} → {}", a.locator),
        },
...
mod tests {
    ...
    fn each_of_the_four_parses() {
        assert_eq!(ActionKind::parse("open_pr"), Some(ActionKind::OpenPr));
        assert_eq!(ActionKind::parse("push_to_pr"), Some(ActionKind::PushToPr));
        assert_eq!(
            ActionKind::parse("bot_thread_op"),
            Some(ActionKind::BotThreadOp)
        );
        assert_eq!(
            ActionKind::parse("linear_write"),
            Some(ActionKind::LinearWrite)
        );
        assert_eq!(ActionKind::parse("slack_send"), None);
    }
    ...
    /// V10, as it reads after merge note 3: an UNSPELLABLE kind is refused BY THE EXECUTOR, and
    /// the refusal names the vocabulary it is not in. This used to be refused one step earlier by
    /// this crate's own `parse_kind`, which is what V10 recorded as its open item.
    #[tokio::test]
    async fn a_spelling_outside_the_four_is_refused_by_the_executor() {
        let cx = cx();
        let out = execute_all(
            &cx,
            &[RuntimeAction::Act {
        ...
        match &out[0] {
            ActionOutcome::Refused { reason } => {
                assert!(reason.contains("`slack_send`"), "{reason}");
                assert!(
                    reason.contains("open_pr, push_to_pr, bot_thread_op, linear_write"),
                    "the refusal names the four the harness CAN do: {reason}"
                );
            }
            other => panic!("expected a refusal, got {other:?}"),
        }
