...
impl ActionProvider for LinearActions {
    ...
    async fn execute(&self, req: &ExecuteRequest) -> Result<ActionArtifact, ActionError> {
        self.write(req).await.map_err(linear_error)
    }

    /// Reconciliation's half: is the marker on one of the issue's comments? A READ, always.
    ///
    /// MERGE (note 2): a method on `ActionProvider` now, not a second trait with a registry of
    /// its own.
...
/// One place this Provider's own error becomes the seam's. Merge note 4: a payload §7 does not
/// sanction — both `status` and `comment`, neither, a title, a new issue — is a REFUSAL
/// (`ActionError::BadPayload`), not a provider malfunction. Transport and server failures stay
/// `Provider { source }`, which is what that variant means.
fn linear_error(e: LinearActionError) -> ActionError {
    let kind = ActionKind::LinearWrite.as_str();
    match e {
        // The RULE travels with the detail: `ActionError::BadPayload`'s own sentence names the
        // kind and §7, and the caller still has to be told which payload rule was broken.
        LinearActionError::BadPayload { detail } => ActionError::BadPayload {
            kind,
            detail: format!("exactly one of `status` or `comment` must be set ({detail})"),
        },
        LinearActionError::Creation { field } => ActionError::BadPayload {
            kind,
            detail: format!("creating tickets is Andrey's, not the harness's (`{field}`)"),
        },
        other => ActionError::Provider {
            kind,
...
impl Plugin for LinearActionsPlugin {
    ...
    fn inject() -> Inject {
        ...
        Inject::required(["actions", "ledger"])
    }
    ...
    async fn apply(ctx: Context, cfg: Arc<Self::Config>) -> Result<(), PluginError> {
        ...
        ActionsHandle::provider(&actions, &ctx, provider as Arc<dyn ActionProvider>).await?;
        Ok(())
    }
