...
/// What a refusal SAYS. MERGE (note 4): a payload §7 does not sanction is `BadPayload` — a
/// refusal — while a transport or endpoint failure stays `Provider { source }`.
...
/// Merge note 4, stated once: every payload refusal this Provider makes reads as a REFUSAL and not
/// as a malfunction. Both of §7's payload rules are covered — "exactly one of status/comment" and
/// "creating tickets is Andrey's".
#[tokio::test]
async fn a_payload_refusal_is_a_refusal_and_not_a_provider_malfunction() {
    for payload in [
        serde_json::json!({ "status": "Done", "comment": "hi" }),
        serde_json::json!({ "title": "a new ticket", "comment": "hi" }),
    ] {
        let p = LinearActions::with_api(Arc::new(FakeLinear::default()));
        let e = p
            .execute(&exec(payload.clone()))
            .await
            .expect_err("refused");
        assert!(
            matches!(e, bough_plugin_actions::ActionError::BadPayload { kind, .. } if kind == "linear_write"),
            "for {payload}: {e}"
        );
    }
}
...
async fn find_marker_reads_the_issues_comments_and_writes_nothing() {
    use bough_plugin_actions::ActionProvider;
    let api = Arc::new(FakeLinear::default());
    let p = LinearActions::with_api(api.clone());
