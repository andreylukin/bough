        ...
        match cfg.at {
            ...
            FaultSite::WakeStopping => {
                ...
                ctx.on_serial::<bough_plugin_agents::AgentWakeStopping, _, _>(move |p| {
                    let cfg = cfg.clone();
                    async move {
                        // MERGE (H-C6 closed): the NAME comes off the live handle the payload
                        // already carries. It used to be `AgentName::new(p.agent.as_str())` —
                        // `p.agent` is an `AgentId` (a uuid), so `agent: sol` matched nothing at
                        // this site and every configured filter silently fired on every agent.
                        let agent = p.handle.name().clone();
