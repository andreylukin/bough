...
pub use delivery::{delivery_of, envelope_of};
...
pub enum CollectError {
    ...
    /// The router refused or failed to deliver. MERGE (track B → Phase 5): the collectors hand
    /// their cited mail to `mail-router` when one is in the tree, and the router's own refusal is
...
pub mod refs {
    ...
    /// `repo:o/r` — the SCOPE ref every GitHub item carries beside its own.
    ///
    /// MERGE (track B → Phase 5): a collector's per-item ref (`gh:o/r#12`) is what the dedupe
    /// guard and the citation key on, and it is useless as a ROUTING key — nobody links a lane to
    /// a pull request that does not exist yet. `mail-router` matches an agent's `routing_refs`
    /// against the envelope's refs, so an item has to carry the thing a person would actually
    /// link a lane to: the repository.
    pub fn repo(repo: &str) -> Ref {
        Ref::new(format!("repo:{repo}"))
    }

    /// `team:TEAM` — the same idea for Linear, derived from the issue key's prefix.
    pub fn team(key: &str) -> Ref {
        Ref::new(format!("team:{}", key.split('-').next().unwrap_or(key)))
    }
