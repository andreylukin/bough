...
pub async fn fixture() -> (Context, LedgerHandle, ActionsHandle) {
    ...
    // `actions-reconcile` surfaces an unfindable intent as a DRAFT on the agent's own chain, so
    // the fixture declares the draft step types the way the `drafts` row does in a tree.
...
/// What a refusal SAYS, whichever of the seam's two refusal shapes it took.
///
/// MERGE (note 4): a payload §7 does not sanction is now `ActionError::BadPayload` — a REFUSAL —
/// while a transport or upstream failure stays `ActionError::Provider { source }`. Both are
/// refusals from the caller's point of view, so this helper reads either; `classify` below is
/// what pins which one a given case must be.
pub fn refusal(e: bough_plugin_actions::ActionError) -> String {
    match e {
        ...
        bough_plugin_actions::ActionError::BadPayload { detail, .. } => detail,
        other => panic!("expected a refusal, got {other}"),
    }
}

/// `"bad_payload"` or `"provider"` — merge note 4's distinction, as one word a test can assert on.
pub fn classify(e: &bough_plugin_actions::ActionError) -> &'static str {
    match e {
        bough_plugin_actions::ActionError::BadPayload { .. } => "bad_payload",
        bough_plugin_actions::ActionError::Provider { .. } => "provider",
        _ => "other",
    }
}
