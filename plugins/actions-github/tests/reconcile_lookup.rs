...
//! `ArtifactLookup`, which can only prove the reconciler's own branching. This file instead wires
//! the real `GithubActions` — the same type that would have executed the action — as the lookup,
...
//! is asserted on the recorded argv log rather than on the absence of a trait method.

mod support;

use std::sync::Arc;

use bough_plugin_actions::{idem_key, marker_for, ActionKind, ActionsHandle};
...
use bough_plugin_ledger::{
    ActionQuery, ActionStatus, IdemKey, LedgerHandle, NewAction, StepId, TrajId, WakeId,
};
...
fn reconcile_over(
    ...
    futures::executor::block_on(async {
        registry
            .register(&ctx, provider(gh) as Arc<dyn ArtifactLookup>)
            .await
            .expect("the real Provider registers its lookup half");
    });
    (
        Reconciler::new(
...
async fn the_real_provider_finds_the_marker_in_the_world_and_the_row_is_marked_done() {
    ...
    let (r, _reg) = reconcile_over(&actions, &gh, drafts.clone());
...
async fn the_real_provider_finds_no_marker_so_it_is_surfaced_as_a_draft_and_never_re_executed() {
    ...
    assert!(drafted[0].body.contains(&marker_for(&idem)));
