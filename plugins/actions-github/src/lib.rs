...
impl ActionProvider for GithubActions {
    ...
    async fn execute(&self, req: &ExecuteRequest) -> Result<ActionArtifact, ActionError> {
        ...
        out.map_err(|e| gh_error(kind.as_str(), e))
    }
    ...
    /// registry of its own. One Provider, one registration, one place `open_pr` lives.
    async fn find_marker(
        ...
        self.find(kind, canonical_target, marker)
            .await
            .map_err(|e| gh_error(kind.as_str(), e))
    }
}

/// One place the Provider's own error becomes the seam's. Merge note 4: a payload §7 does not
/// sanction is a REFUSAL (`ActionError::BadPayload`) and not a provider malfunction — everything
/// else is the transport failing, which is what `Provider { source }` means.
fn gh_error(kind: &'static str, e: GhActionError) -> ActionError {
    match e {
        GhActionError::BadPayload { kind, detail } => ActionError::BadPayload { kind, detail },
...
impl Plugin for GithubActionsPlugin {
    ...
    /// Register the Provider on `ctx.actions` as an effect. ONE registration: `find_marker` is a
    /// method on `ActionProvider` now (merge note 2), so the lookup half cannot go missing.
