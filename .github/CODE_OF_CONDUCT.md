# Code of Conduct

## The standard

Be respectful and assume good faith. Everyone participates here regardless of age, body
size, disability, ethnicity, sex characteristics, gender identity and expression, level
of experience, education, socio-economic status, nationality, appearance, race, caste,
color, religion, or sexual identity and orientation.

Specifically:

- Give feedback on the work, not the person. Say what would make a change mergeable.
- Accept feedback the same way. Disagreement about a design is normal and welcome.
- When you're wrong, say so and move on. No one is keeping score.

Not acceptable, anywhere in the project:

- Harassment, public or private; sexualized language, imagery, or attention.
- Personal attacks, insults, trolling, or political flamewars.
- Publishing someone's private information without permission.
- Sustained disruption: relitigating settled decisions, or wearing people down.

## Scope

All project spaces (issues, PRs, discussions, commits), and anywhere you're representing
bough in public.

## Enforcement

Report to **luk.andrey@gmail.com**. Reports are handled privately and promptly, and the
reporter's identity is protected.

Maintainers may edit, remove, or reject any contribution that violates this document, and
will say why. Responses escalate as the situation warrants: a private warning, a temporary
ban from interaction, or a permanent ban. Severity and pattern both count; one serious
incident is enough, and so is a persistent low-grade one.

Maintainers who violate this document face the same consequences, not fewer.

## Attribution

Adapted and condensed from the [Contributor Covenant][cc] v2.1. The full text, including
its detailed enforcement ladder, is the reference if anything here is ambiguous.

[cc]: https://www.contributor-covenant.org/version/2/1/code_of_conduct.html
