...
import { checkSyntax, clip, codeGist } from "./text.ts";
...
export async function startWorkflow(ctx: WorkflowCtx, opts: StartOpts): Promise<WorkflowRun> {
  ...
  const bad = checkSyntax(
    opts.script.replace(/export\s+const\s+meta\s*=/, "const meta ="),
    ["agent", "parallel", "pipeline", "phase", "log", "args", "console"],
    "workflow script",
  );
