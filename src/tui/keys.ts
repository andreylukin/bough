...
export const KEYMAP: { section: string; keys: [string, string][] }[] = [
  {
    ...
    keys: [
      ...
      ["^e", "empty draft: expand/collapse all tool calls · while typing: end of line"],
      ["^j", "insert a newline"],
      ["^a ^b ^f ^d", "start of line · back · forward · delete char (while typing)"],
      ["⌥b ⌥f", "back/forward a word (also ⌥←/→)"],
  ...
  },
  {
    ...
    keys: [
      ["^p ^o ^t", "sessions · model · toggle — any time, even mid-draft"],
      ["^f ^d ^b", "conversation · changes · jobs — empty draft only (readline chords)"],
      ["enter", "open / act on the selected row"],
      ["esc", "back out one level · then out of the panel"],
      ["x", "the destructive action here — the irreversible ones ask twice"],
      ["/", "filter"],
      ["j/k or ↑↓", "move"],
    ],
  },
