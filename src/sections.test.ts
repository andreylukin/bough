...
Deno.test("parseSections tolerates fences and prose around the JSON", () => {
  const raw = 'Here you go:\n```json\n[{"start":0,"end":1,"kind":"debug","label":"auth"}]\n```';
  assertEquals(parseSections(raw), [{ start: 0, end: 1, kind: "debug", label: "auth" }]);
  assertEquals(parseSections("no json here"), null);
  assertEquals(parseSections('[{"start":0,"end":1,"kind":"nonsense","label":"x"}]'), null);
});

Deno.test("normalizeSections clips, fills gaps, and drops overlaps", () => {
  const out = normalizeSections(
    [
      { start: 4, end: 9, kind: "review", label: "ship it" }, // clipped to n-1
      { start: 0, end: 1, kind: "debug", label: "auth" },
      { start: 1, end: 2, kind: "implement", label: "overlap trimmed" },
  ...
  assertEquals(out, [
    { start: 0, end: 1, kind: "debug", label: "auth" },
    { start: 2, end: 2, kind: "implement", label: "overlap trimmed" },
    { start: 3, end: 3, kind: "discuss", label: "…" },
    { start: 4, end: 5, kind: "review", label: "ship it" },
  ]);
});

Deno.test("normalizeSections covers everything when the model returns nothing usable", () => {
  assertEquals(normalizeSections([], 3), [{ start: 0, end: 2, kind: "discuss", label: "…" }]);
});

Deno.test("sectionize prompts with numbered gists and normalizes the reply", async () => {
  const llm = fakeLlm(['[{"start":0,"end":0,"kind":"config","label":"setup"}]']);
  const out = await sectionize({ llm }, [{ gist: "install deps" }, { gist: "fix the test" }]);
  assertEquals(out, [
    { start: 0, end: 0, kind: "config", label: "setup" },
    { start: 1, end: 1, kind: "discuss", label: "…" },
  ]);
