...
const SYSTEM = [
  ...
  "When the request quotes exact expected output or file content, the check must",
  "compare against that text copied VERBATIM from the request — never retyped and",
  "never re-derived by running your own code: an expectation computed from your",
  "implementation inherits its bugs (printing `1.0` where the spec shows `1` and",
  "concluding it matches). The spec's literal bytes are the contract.",
